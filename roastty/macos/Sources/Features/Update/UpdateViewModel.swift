...
enum UpdateState: Equatable {
    ...
    struct UpdateAvailable {
        ...
        var releaseNotes: ReleaseNotes? {
            let currentCommit = Bundle.main.infoDictionary?["RoasttyCommit"] as? String
    ...
    enum ReleaseNotes {
        ...
        init?(displayVersionString: String, currentCommit: String?) {
            ...
            if let semver = Self.extractSemanticVersion(from: version) {
                ...
                if let url = URL(string: "https://roastty.org/docs/install/release-notes/\(slug)") {
                    self = .tagged(url)
                    return
                }
            }
            ...
            if let currentHash = currentCommit, !currentHash.isEmpty,
               let url = URL(string: "https://github.com/roastty-org/roastty/compare/\(currentHash)...\(newHash)") {
                self = .compareTip(url)
            } else if let url = URL(string: "https://github.com/roastty-org/roastty/commit/\(newHash)") {
                self = .commit(url)
            } else {
                return nil
            }
    ...
    struct Installing {
        /// True if this state is triggered by ``Roastty/UpdateDriver/updater(_:willInstallUpdateOnQuit:immediateInstallationBlock:)``
