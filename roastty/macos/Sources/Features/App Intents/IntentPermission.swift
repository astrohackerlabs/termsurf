...
/// Requests permission for Shortcuts app to interact with Roastty
///
/// This function displays a permission dialog asking the user to allow Shortcuts
/// to interact with Roastty. The permission is automatically cached for 10 minutes
...
/// permission for one intent allows all Roastty intents to execute without additional
    ...
    await withCheckedContinuation { continuation in
        Task { @MainActor in
            if let delegate = NSApp.delegate as? AppDelegate {
                switch delegate.roastty.config.macosShortcuts {
                case .allow:
            ...
            PermissionRequest.show(
                "com.mitchellh.roastty.shortcutsPermission",
                message: "Allow Shortcuts to interact with Roastty?",
