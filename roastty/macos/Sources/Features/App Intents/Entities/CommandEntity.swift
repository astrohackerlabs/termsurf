...
struct CommandEntity: AppEntity {
    ...
    init(_ command: Roastty.Command, for terminal: TerminalEntity) {
...
struct CommandQuery: EntityQuery {
    ...
    func entities(for identifiers: [CommandEntity.ID]) async throws -> [CommandEntity] {
        ...
        let commands = appDelegate.roastty.config.commandPaletteEntries
    ...
    func suggestedEntities() async throws -> [CommandEntity] {
        ...
        return appDelegate.roastty.config.commandPaletteEntries.map { CommandEntity($0, for: terminal) }
    }
}
