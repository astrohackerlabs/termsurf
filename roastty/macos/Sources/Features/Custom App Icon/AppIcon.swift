...
/// The icon style for the Roastty App.
enum AppIcon: Equatable, Codable, Sendable {
    ...
    case customStyle(_ icon: ColorizedRoasttyIcon)

#if !DOCK_TILE_PLUGIN
    init?(config: Roastty.Config) {
        ...
        case .customStyle:
            ...
            self = .customStyle(ColorizedRoasttyIcon(screenColors: screenColors, ghostColor: ghostColor, frame: config.macosIconFrame))
        }
    }
#endif
...
actor AppIconUpdater {
    func update(icon: AppIcon?) {
        UserDefaults.roastty.appIcon = icon
        // Notify DockTilePlugin to update dock icon
        DistributedNotificationCenter.default()
            .postNotificationName(
                .roasttyIconDidChange,
                object: nil,
                userInfo: nil,
