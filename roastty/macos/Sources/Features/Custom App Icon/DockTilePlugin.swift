...
class DockTilePlugin: NSObject, NSDockTilePlugIn {
    // WARNING: An instance of this class is alive as long as Roastty's icon is
    ...
    #if DEBUG
    private let roasttyUserDefaults = UserDefaults(suiteName: "com.mitchellh.roastty.debug")
    #else
    private let roasttyUserDefaults = UserDefaults(suiteName: "com.mitchellh.roastty")
    #endif
    ...
    func setDockTile(_ dockTile: NSDockTile?) {
        // If no dock tile or no access to Roastty defaults, we can't do anything.
        guard let dockTile, let roasttyUserDefaults else {
            iconChangeObserver = nil
            return
        }

        // Try to restore the previous icon on launch.
        iconDidChange(roasttyUserDefaults.appIcon, dockTile: dockTile)

        // Setup a new observer for when the icon changes so we can update. This message
        // is sent by the primary Roastty app.
        iconChangeObserver = DistributedNotificationCenter
            .default()
            .publisher(for: .roasttyIconDidChange)
            .map { [weak self] _ in self?.roasttyUserDefaults?.appIcon }
