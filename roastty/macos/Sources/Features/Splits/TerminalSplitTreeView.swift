...
enum TerminalSplitOperation {
    ...
    struct Resize {
        let node: SplitTree<Roastty.SurfaceView>.Node
        let ratio: Double
    }

    struct Drop {
        /// The surface being dragged.
        let payload: Roastty.SurfaceView

        /// The surface it was dragged onto
        let destination: Roastty.SurfaceView
...
struct TerminalSplitTreeView: View {
    let tree: SplitTree<Roastty.SurfaceView>
    ...
    var body: some View {
        if let node = tree.zoomed ?? tree.root {
            ...
            // See: https://github.com/roastty-org/roastty/issues/7546
    ...
    var body: some View {
        ...
        case .split(let split):
            ...
            SplitView(
                ...
                dividerColor: roastty.config.splitDividerColor,
...
private struct TerminalSplitLeaf: View {
    let surfaceView: Roastty.SurfaceView
    let isSplit: Bool
    let action: (TerminalSplitOperation) -> Void
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .background {
                ...
                if !isSelfDragging {
                    Color.clear
                        .onDrop(of: [.roasttySurfaceId], delegate: SplitDropDelegate(
                            dropState: $dropState,
            ...
            .onPreferenceChange(Roastty.DraggingSurfaceKey.self) { value in
    ...
    private struct SplitDropDelegate: DropDelegate {
        ...
        let viewSize: CGSize
        let destinationSurface: Roastty.SurfaceView
        let action: (TerminalSplitOperation) -> Void

        func validateDrop(info: DropInfo) -> Bool {
            info.hasItemsConforming(to: [.roasttySurfaceId])
        }
        ...
        func performDrop(info: DropInfo) -> Bool {
            ...
            let providers = info.itemProviders(for: [.roasttySurfaceId])
            guard let provider = providers.first else { return false }

            // Capture action before the async closure
            _ = provider.loadTransferable(type: Roastty.SurfaceView.self) { [weak destinationSurface] result in
                switch result {
