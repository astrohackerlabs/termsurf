...
/// Handler for the `send mouse button` AppleScript command defined in `Roastty.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="RoasttyScriptMouseButtonCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(RoasttyScriptMouseButtonCommand)
final class ScriptMouseButtonCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let action: Roastty.Input.MouseState
...
/// Four-character codes matching the `mouse button` enumeration in `Roastty.sdef`.
private enum ScriptMouseButtonValue {
    ...
    var roasttyButton: Roastty.Input.MouseButton {
        switch self {
        case .left: .left
        case .right: .right
        case .middle: .middle
