...
/// AppleScript-facing wrapper around a live Roastty terminal surface.
///
...
///
/// Mapping from `Roastty.sdef`:
/// - `class terminal` -> this class (`@objc(RoasttyAppleScriptTerminal)`).
...
final class ScriptTerminal: NSObject {
    ...
    weak var surfaceView: Roastty.SurfaceView?

    init(surfaceView: Roastty.SurfaceView) {
    ...
    func perform(action: String) -> Bool {
        ...
        if action == "ui_test_quicklook" {
            guard ProcessInfo.processInfo.environment["ROASTTY_UI_TEST_ENABLE_QUICKLOOK_ACTION"] == "1" else { return false }
    ...
    func handleSplit(_ command: NSScriptCommand) -> Any? {
        ...
        let baseConfig: Roastty.SurfaceConfiguration?
        if let scriptRecord = command.evaluatedArguments?["configuration"] as? NSDictionary {
            do {
                baseConfig = try Roastty.SurfaceConfiguration(scriptRecord: scriptRecord)
            } catch {
...
/// Converts four-character codes from the `split direction` enumeration in `Roastty.sdef`
...
enum ScriptSplitDirection {
    ...
    var splitDirection: SplitTree<Roastty.SurfaceView>.NewDirection {
        switch self {
        case .right: .right
        case .left: .left
        case .down: .down
