...
// Application-level Cocoa scripting hooks for the Roastty AppleScript dictionary.
//
...
// the runtime object behind the `application` class in `Roastty.sdef`.
...
extension NSApplication {
    ...
    /// the frontmost logical Roastty window.
    @objc(frontWindow)
...
extension NSApplication {
    ...
    func handleNewSurfaceConfigurationScriptCommand(_ command: NSScriptCommand) -> NSDictionary? {
        ...
        do {
            let configuration = try Roastty.SurfaceConfiguration(
    ...
    func handleNewWindowScriptCommand(_ command: NSScriptCommand) -> ScriptWindow? {
        ...
        guard let appDelegate = delegate as? AppDelegate else {
            ...
            command.scriptErrorString = "Roastty app delegate is unavailable."
            return nil
        }
...
@MainActor
extension NSApplication {
    /// Whether Roastty should currently accept AppleScript interactions.
    var isAppleScriptEnabled: Bool {
        guard let appDelegate = delegate as? AppDelegate else { return true }
        return appDelegate.roastty.config.macosAppleScript
    }
    ...
    fileprivate var allSurfaceViews: [Roastty.SurfaceView] {
