...
/// Represents the Roastty `quick-terminal-size` configuration. See the documentation for
...
struct QuickTerminalSize {
    ...
    init(from cStruct: roastty_config_quick_terminal_size_s) {
    ...
    enum Size {
        ...
        init?(from cStruct: roastty_quick_terminal_size_s) {
            switch cStruct.tag {
            case ROASTTY_QUICK_TERMINAL_SIZE_NONE:
                return nil
            case ROASTTY_QUICK_TERMINAL_SIZE_PERCENTAGE:
                self = .percentage(cStruct.value.percentage)
            case ROASTTY_QUICK_TERMINAL_SIZE_PIXELS:
