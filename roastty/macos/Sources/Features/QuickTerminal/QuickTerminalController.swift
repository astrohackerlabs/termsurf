...
class QuickTerminalController: BaseTerminalController {
    ...
    /// The previously running application when the terminal is shown. This is NEVER Roastty.
    ...
    init(_ roastty: Roastty.App,
        ...
        super.init(roastty, baseConfig: base, surfaceTree: .init())
    ...
    override func focusSurface(_ view: Roastty.SurfaceView) {
        if visible {
        ...
        if surfaceTree.isEmpty,
           let roastty_app = roastty.app {
            if let tree = restorationState?.surfaceTree, !tree.isEmpty {
                ...
                config.environmentVariables["ROASTTY_QUICK_TERMINAL"] = "1"

                let view = Roastty.SurfaceView(roastty_app, baseConfig: config)
        ...
        NSAnimationContext.runAnimationGroup({ context in
            ...
            DispatchQueue.main.async {
                ...
                if !NSApp.isActive {
                    ...
                    // shown on a screen that has no other Roastty windows, it takes
                    // a few (variable) event loop ticks until we can actually focus it.
                    // https://github.com/roastty-org/roastty/issues/2409
                    //
    ...
    override func syncAppearance() {
        ...
        if !isBackgroundOpaque && (self.derivedConfig.backgroundOpacity < 1 || derivedConfig.backgroundBlur.isGlassStyle) {
            ...
            if !derivedConfig.backgroundBlur.isGlassStyle {
                roastty_set_window_background_blur(roastty.app, Unmanaged.passUnretained(window).toOpaque())
            }
        } else {
        ...
        terminalViewContainer?.roasttyConfigDidChange(roastty.config, preferredBackgroundColor: nil)
    }
    ...
    @objc private func roasttyConfigDidChange(_ notification: Notification) {
        ...
        terminalViewContainer?.roasttyConfigDidChange(config, preferredBackgroundColor: nil)
    }
