...
/// A base class for windows that can contain Roastty windows. This base class implements
/// the bare minimum functionality that every terminal window in Roastty should implement.
///
...
class BaseTerminalController: NSWindowController,
    ...
    let roastty: Roastty.App

    /// The currently focused surface.
    var focusedSurface: Roastty.SurfaceView? {
        didSet { syncFocusToSurfaceTree() }
    }

    /// The tree of splits within this terminal window.
    @Published var surfaceTree: SplitTree<Roastty.SurfaceView> = .init() {
    ...
    private var appliedColorScheme: roastty_color_scheme_e?

    /// The configuration derived from the Roastty config so we don't need to rely on references.
    ...
    init(_ roastty: Roastty.App,
         baseConfig base: Roastty.SurfaceConfiguration? = nil,
         surfaceTree tree: SplitTree<Roastty.SurfaceView>? = nil
    ) {
        self.roastty = roastty
        self.derivedConfig = DerivedConfig(roastty.config)

        super.init(window: nil)

        // Initialize our initial surface.
        guard let roastty_app = roastty.app else { preconditionFailure("app must be loaded") }
        self.surfaceTree = tree ?? .init(view: Roastty.SurfaceView(roastty_app, baseConfig: base))
        ...
        center.addObserver(
            ...
            name: Roastty.Notification.confirmClipboard,
            object: nil)
        center.addObserver(
            self,
        ...
        center.addObserver(
            self,
            selector: #selector(roasttyConfigDidChangeBase(_:)),
            name: .roasttyConfigDidChange,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(roasttyCommandPaletteDidToggle(_:)),
            name: .roasttyCommandPaletteDidToggle,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(roasttyMaximizeDidToggle(_:)),
        ...
        center.addObserver(
            self,
            selector: #selector(roasttyDidCloseSurface(_:)),
            name: Roastty.Notification.roasttyCloseSurface,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(roasttyDidNewSplit(_:)),
            name: Roastty.Notification.roasttyNewSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(roasttyDidEqualizeSplits(_:)),
            name: Roastty.Notification.didEqualizeSplits,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(roasttyDidFocusSplit(_:)),
            name: Roastty.Notification.roasttyFocusSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(roasttyDidToggleSplitZoom(_:)),
            name: Roastty.Notification.didToggleSplitZoom,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(roasttyDidResizeSplit(_:)),
            name: Roastty.Notification.didResizeSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(roasttyDidPresentTerminal(_:)),
            name: Roastty.Notification.roasttyPresentTerminal,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(roasttySurfaceDragEndedNoTarget(_:)),
            name: .roasttySurfaceDragEndedNoTarget,
            object: nil)
    ...
    @discardableResult
    func newSplit(
        at oldView: Roastty.SurfaceView,
        direction: SplitTree<Roastty.SurfaceView>.NewDirection,
        baseConfig config: Roastty.SurfaceConfiguration? = nil
    ) -> Roastty.SurfaceView? {
        // We can only create new splits for surfaces in our tree.
        guard surfaceTree.root?.node(view: oldView) != nil else { return nil }

        // Create a new surface view
        guard let roastty_app = roastty.app else { return nil }
        let newView = Roastty.SurfaceView(roastty_app, baseConfig: config)

        // Do the split
        let newTree: SplitTree<Roastty.SurfaceView>
        do {
            ...
            // no big deal.
            Roastty.logger.warning("failed to insert split: \(error)")
            return nil
        }
    ...
    func surfaceTreeDidChange(from: SplitTree<Roastty.SurfaceView>, to: SplitTree<Roastty.SurfaceView>) {
    ...
    /// Update all surfaces with the focus state. This ensures that libroastty has an accurate view about
    ...
    func closeSurface(
        _ node: SplitTree<Roastty.SurfaceView>.Node,
        withConfirmation: Bool = true
    ) {
        ...
        // due to SwiftUI bugs (see Roastty #560). To repeat from #560, the bug is that
    ...
    private func findNextFocusTargetAfterClosing(node: SplitTree<Roastty.SurfaceView>.Node) -> Roastty.SurfaceView? {
    ...
    private func removeSurfaceNode(_ node: SplitTree<Roastty.SurfaceView>.Node) {
        // Move focus if the closed surface was focused and we have a next target
        let nextFocus: Roastty.SurfaceView? = if node.contains(
    ...
    func replaceSurfaceTree(
        _ newTree: SplitTree<Roastty.SurfaceView>,
        moveFocusTo newView: Roastty.SurfaceView? = nil,
        moveFocusFrom oldView: Roastty.SurfaceView? = nil,
        ...
        if let newView {
            DispatchQueue.main.async {
                Roastty.moveFocus(to: newView, from: oldView)
            }
        }
        ...
        undoManager.registerUndo(
            ...
            if let oldView {
                DispatchQueue.main.async {
                    Roastty.moveFocus(to: oldView, from: target.focusedSurface)
                }
            }
    ...
    @objc private func roasttyConfigDidChangeBase(_ notification: Notification) {
        ...
        guard let config = notification.userInfo?[
            Notification.Name.RoasttyConfigChangeKey
        ] as? Roastty.Config else { return }
    ...
    @objc private func roasttyCommandPaletteDidToggle(_ notification: Notification) {
        guard let surfaceView = notification.object as? Roastty.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        toggleCommandPalette(nil)
    }

    @objc private func roasttyMaximizeDidToggle(_ notification: Notification) {
        guard let window else { return }
        guard let surfaceView = notification.object as? Roastty.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        window.zoom(nil)
    ...
    @objc private func roasttyDidCloseSurface(_ notification: Notification) {
        guard let target = notification.object as? Roastty.SurfaceView else { return }
    ...
    @objc private func roasttyDidNewSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let oldView = notification.object as? Roastty.SurfaceView else { return }
        guard surfaceTree.root?.node(view: oldView) != nil else { return }

        // Notification must contain our base config
        let configAny = notification.userInfo?[Roastty.Notification.NewSurfaceConfigKey]
        let config = configAny as? Roastty.SurfaceConfiguration

        // Determine our desired direction
        guard let directionAny = notification.userInfo?["direction"] else { return }
        guard let direction = directionAny as? roastty_action_split_direction_e else { return }
        let splitDirection: SplitTree<Roastty.SurfaceView>.NewDirection
        switch direction {
        case ROASTTY_SPLIT_DIRECTION_RIGHT: splitDirection = .right
        case ROASTTY_SPLIT_DIRECTION_LEFT: splitDirection = .left
        case ROASTTY_SPLIT_DIRECTION_DOWN: splitDirection = .down
        case ROASTTY_SPLIT_DIRECTION_UP: splitDirection = .up
        default: return
        }
    ...
    @objc private func roasttyDidEqualizeSplits(_ notification: Notification) {
        guard let target = notification.object as? Roastty.SurfaceView else { return }
    ...
    @objc private func roasttyDidFocusSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? Roastty.SurfaceView else { return }
        guard surfaceTree.root?.node(view: target) != nil else { return }

        // Get the direction from the notification
        guard let directionAny = notification.userInfo?[Roastty.Notification.SplitDirectionKey] else { return }
        guard let direction = directionAny as? Roastty.SplitFocusDirection else { return }
        ...
        DispatchQueue.main.async {
            Roastty.moveFocus(to: nextSurface, from: target)
        }
    }

    @objc private func roasttyDidToggleSplitZoom(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? Roastty.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
        ...
        DispatchQueue.main.async {
            Roastty.moveFocus(to: target)
        }
    }

    @objc private func roasttyDidResizeSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? Roastty.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }

        // Extract direction and amount from notification
        guard let directionAny = notification.userInfo?[Roastty.Notification.ResizeSplitDirectionKey] else { return }
        guard let direction = directionAny as? Roastty.SplitResizeDirection else { return }

        guard let amountAny = notification.userInfo?[Roastty.Notification.ResizeSplitAmountKey] else { return }
        guard let amount = amountAny as? UInt16 else { return }

        // Convert Roastty.SplitResizeDirection to SplitTree.Spatial.Direction
        let spatialDirection: SplitTree<Roastty.SurfaceView>.Spatial.Direction
        switch direction {
        ...
        do {
            ...
            Roastty.logger.warning("failed to resize split: \(error)")
        }
    }

    @objc private func roasttyDidPresentTerminal(_ notification: Notification) {
        guard let target = notification.object as? Roastty.SurfaceView else { return }
        guard surfaceTree.contains(target) else { return }
        ...
        Roastty.moveFocus(to: target)
        Roastty.moveFocus(to: target, delay: 0.1)

        // Show a brief highlight to help the user locate the presented terminal.
        target.highlight()
    }

    @objc private func roasttySurfaceDragEndedNoTarget(_ notification: Notification) {
        guard let target = notification.object as? Roastty.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
        ...
        let newTree = SplitTree<Roastty.SurfaceView>(view: target)
        ...
        _ = TerminalController.newWindow(
            roastty,
            tree: newTree,
            position: notification.userInfo?[Notification.Name.roasttySurfaceDragEndedNoTargetPointKey] as? NSPoint,
            confirmUndo: false,
    ...
    private func localEventFlagsChanged(_ event: NSEvent) -> NSEvent? {
        var surfaces: [Roastty.SurfaceView] = surfaceTree.map { $0 }
    ...
    func focusedSurfaceDidChange(to: Roastty.SurfaceView?) {
    ...
    private func computeTitle(title: String, bell: Bool) -> String {
        var result = title
        if bell && roastty.config.bellFeatures.contains(.title) {
            result = "🔔 \(result)"
        }
    ...
    private func splitDidResize(node: SplitTree<Roastty.SurfaceView>.Node, to newRatio: Double) {
        let resizedNode = node.resizing(to: newRatio)
        do {
            surfaceTree = try surfaceTree.replacing(node: node, with: resizedNode)
        } catch {
            Roastty.logger.warning("failed to replace node during split resize: \(error)")
        }
    }

    private func splitDidDrop(
        source: Roastty.SurfaceView,
        destination: Roastty.SurfaceView,
        zone: TerminalSplitDropZone
    ) {
        // Map drop zone to split direction
        let direction: SplitTree<Roastty.SurfaceView>.NewDirection = switch zone {
        case .top: .up
        case .bottom: .down
        case .left: .left
        case .right: .right
        ...
        if let sourceNode = surfaceTree.root?.node(view: source) {
            ...
            do {
                ...
                Roastty.logger.warning("failed to insert surface during drop: \(error)")
                return
            }
        ...
        var sourceNode: SplitTree<Roastty.SurfaceView>.Node?
        ...
        guard let sourceController, let sourceNode else {
            Roastty.logger.warning("source surface not found in any window during drop")
            return
        }
        ...
        do {
            ...
            Roastty.logger.warning("failed to insert surface during cross-window drop: \(error)")
            return
        }
    ...
    func performAction(_ action: String, on surfaceView: Roastty.SurfaceView) {
        guard let surface = surfaceView.surface else { return }
        let len = action.utf8CString.count
        if len == 0 { return }
        _ = action.withCString { cString in
            roastty_surface_binding_action(surface, cString, UInt(len - 1))
        }
    }
    ...
    func toggleBackgroundOpacity() {
        ...
        guard roastty.config.backgroundOpacity < 1 else { return }
    ...
    @objc private func onConfirmClipboardRequest(notification: SwiftUI.Notification) {
        ...
        guard let str = notification.userInfo?[Roastty.Notification.ConfirmClipboardStrKey] as? String else { return }
        guard let state = notification.userInfo?[Roastty.Notification.ConfirmClipboardStateKey] as? UnsafeMutableRawPointer? else { return }
        guard let request = notification.userInfo?[Roastty.Notification.ConfirmClipboardRequestKey] as? Roastty.ClipboardRequest else { return }

        // If we already have a clipboard confirmation view up, we ignore this request.
        // This shouldn't be possible...
        guard self.clipboardConfirmation == nil else {
            Roastty.App.completeClipboardRequest(surface, data: "", state: state, confirmed: true)
            return
        }
    ...
    func clipboardConfirmationComplete(_ action: ClipboardConfirmationView.Action, _ request: Roastty.ClipboardRequest) {
        ...
        case .osc_52_read, .paste:
            ...
            Roastty.App.completeClipboardRequest(cc.surface, data: str, state: cc.state, confirmed: true)
        }
    }
    ...
    func windowDidBecomeKey(_ notification: Notification) {
        ...
        if let window, window.firstResponder == window, let focusedSurface {
            DispatchQueue.main.async {
                Roastty.moveFocus(to: focusedSurface)
            }
        }
    ...
    private func syncSurfaceTreeOcclusionState() {
        ...
        for view in surfaceTree {
            if let surface = view.surface, view.isWindowVisible != visible {
                roastty_surface_set_occlusion(surface, visible)
    ...
    @IBAction func close(_ sender: Any) {
        ...
        roastty.requestClose(surface: surface)
    }
    ...
    @IBAction func splitRight(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.split(surface: surface, direction: ROASTTY_SPLIT_DIRECTION_RIGHT)
    }

    @IBAction func splitLeft(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.split(surface: surface, direction: ROASTTY_SPLIT_DIRECTION_LEFT)
    }

    @IBAction func splitDown(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.split(surface: surface, direction: ROASTTY_SPLIT_DIRECTION_DOWN)
    }

    @IBAction func splitUp(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.split(surface: surface, direction: ROASTTY_SPLIT_DIRECTION_UP)
    }

    @IBAction func splitZoom(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.splitToggleZoom(surface: surface)
    }
    ...
    @IBAction func equalizeSplits(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.splitEqualize(surface: surface)
    }

    @IBAction func moveSplitDividerUp(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.splitResize(surface: surface, direction: .up, amount: 10)
    }

    @IBAction func moveSplitDividerDown(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.splitResize(surface: surface, direction: .down, amount: 10)
    }

    @IBAction func moveSplitDividerLeft(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.splitResize(surface: surface, direction: .left, amount: 10)
    }

    @IBAction func moveSplitDividerRight(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.splitResize(surface: surface, direction: .right, amount: 10)
    }

    private func splitMoveFocus(direction: Roastty.SplitFocusDirection) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.splitMoveFocus(surface: surface, direction: direction)
    }

    @IBAction func increaseFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.changeFontSize(surface: surface, .increase(1))
    }

    @IBAction func decreaseFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.changeFontSize(surface: surface, .decrease(1))
    }

    @IBAction func resetFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.changeFontSize(surface: surface, .reset)
    }
    ...
    @objc func resetTerminal(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.resetTerminal(surface: surface)
    }
...
extension BaseTerminalController: NSMenuItemValidation {
    ...
    ///
    /// Calling ``roastty_surface_set_color_scheme`` triggers
    ...
    func updateColorSchemeForSurfaceTree() {
        ...
        let scheme: roastty_color_scheme_e
        if themeAppearance.isDark {
            scheme = ROASTTY_COLOR_SCHEME_DARK
        } else {
            scheme = ROASTTY_COLOR_SCHEME_LIGHT
        }
        ...
        for surfaceView in surfaceTree {
            if let surface = surfaceView.surface {
                roastty_surface_set_color_scheme(surface, scheme)
            }
        }
...
extension BaseTerminalController {
    ...
    func surfaceValuesPublisher<Value>(
        valueKeyPath: KeyPath<Roastty.SurfaceView, Value>,
        publisherKeyPath: KeyPath<Roastty.SurfaceView, Published<Value>.Publisher>
    ) -> AnyPublisher<[Roastty.SurfaceView.ID: Value], Never> {
...
extension Notification.Name {
    ...
    static let terminalWindowBellDidChangeNotification = Notification.Name("com.mitchellh.roastty.terminalWindowBellDidChange")
