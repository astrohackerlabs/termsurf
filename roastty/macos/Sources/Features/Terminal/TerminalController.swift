...
class TerminalController: BaseTerminalController, TabGroupCloseCoordinator.Controller {
    override var windowNibName: NSNib.Name? {
        ...
        let config = appDelegate.roastty.config
    ...
    init(_ roastty: Roastty.App,
         withBaseConfig base: Roastty.SurfaceConfiguration? = nil,
         withSurfaceTree tree: SplitTree<Roastty.SurfaceView>? = nil,
         parent: NSWindow? = nil
    ) {
        ...
        super.init(roastty, baseConfig: base, surfaceTree: tree)
        ...
        center.addObserver(
            ...
            name: Roastty.Notification.roasttyToggleFullscreen,
            object: nil)
        center.addObserver(
            self,
        ...
        center.addObserver(
            ...
            name: Roastty.Notification.roasttyGotoTab,
            object: nil)
        center.addObserver(
            self,
    ...
    override func surfaceTreeDidChange(from: SplitTree<Roastty.SurfaceView>, to: SplitTree<Roastty.SurfaceView>) {
    ...
    static func newWindow(
        _ roastty: Roastty.App,
        withBaseConfig baseConfig: Roastty.SurfaceConfiguration? = nil,
        withParent explicitParent: NSWindow? = nil
    ) -> TerminalController {
        let c = TerminalController.init(roastty, withBaseConfig: baseConfig)
        ...
        } else if let fullscreenMode = roastty.config.windowFullscreen {
    ...
    /// - Parameters:
    ///   - roastty: The Roastty app instance.
    ...
    static func newWindow(
        _ roastty: Roastty.App,
        tree: SplitTree<Roastty.SurfaceView>,
        position: NSPoint? = nil,
        confirmUndo: Bool = true,
        inheritBackgroundOpacity: Bool? = nil
    ) -> TerminalController {
        let c = TerminalController.init(roastty, withSurfaceTree: tree)
    ...
    static func newTab(
        ...
        guard let parent,
            ...
            return newWindow(roastty, withBaseConfig: baseConfig, withParent: parent)
        }

        // If our parent is in non-native fullscreen, then new tabs do not work.
        // See: https://github.com/mitchellh/roastty/issues/392
        ...
        let controller = TerminalController.init(roastty, withBaseConfig: baseConfig)
        ...
        if window.tabbingMode != .disallowed {
            ...
            switch roastty.config.windowNewTabPosition {
            case "end":
        ...
        if let undoManager = parentController.undoManager {
            ...
            undoManager.registerUndo(
                ...
                // fixes it.
                // https://github.com/roastty-org/roastty/pull/9512
                DispatchQueue.main.async {
                    undoManager.disableUndoRegistration {
                        target.closeTab(nil)
                    }
    ...
    @objc private func roasttyConfigDidChange(_ notification: Notification) {
        ...
        /// ``Roastty/Roastty/SurfaceView/derivedConfig`` then
    ...
    func relabelTabs() {
        ...
        if let windows = window?.tabbedWindows as? [TerminalWindow] {
            for (tab, window) in zip(1..., windows) {
                ...
                if let equiv = roastty.config.keyboardShortcut(for: "goto_tab:\(tab)") {
    ...
    private func syncAppearance(_ surfaceConfig: Roastty.SurfaceView.DerivedConfig) {
        ...
        terminalViewContainer?.roasttyConfigDidChange(roastty.config, preferredBackgroundColor: window.preferredBackgroundColor)
    }
    ...
    func closeTabImmediately(registerRedo: Bool = true) {
        ...
        if let undoManager, let undoState {
            ...
            undoManager.registerUndo(
                ...
                let newController = TerminalController(roastty, with: undoState)
    ...
    private func registerUndoForCloseWindow() {
        ...
        guard let tabGroup = window.tabGroup,
            ...
            if let undoState {
                ...
                undoManager.registerUndo(
                    withTarget: roastty,
                    expiresAfter: undoExpiration) { roastty in
        ...
        undoManager.registerUndo(
            ...
            let controllers = undoStates.map { undoState in
                TerminalController(roastty, with: undoState)
            }
    ...
    struct UndoState {
        let frame: NSRect
        let surfaceTree: SplitTree<Roastty.SurfaceView>
    ...
    convenience init(_ roastty: Roastty.App, with undoState: UndoState) {
        self.init(roastty, withSurfaceTree: undoState.surfaceTree)
        ...
        if let window {
            ...
            if let focusedUUID = undoState.focusedSurface,
               let focusTarget = surfaceTree.first(where: { $0.id == focusedUUID }) {
                DispatchQueue.main.async {
                    Roastty.moveFocus(to: focusTarget, from: nil)
                }
                ...
                DispatchQueue.main.async {
                    Roastty.moveFocus(to: focusedSurface, from: nil)
                }
            }
        }
    }
    ...
    override func windowDidLoad() {
        ...
        let container = TerminalViewContainer {
            TerminalView(roastty: roastty, viewModel: self, delegate: self)
        }
        ...
        // In various situations, macOS automatically tabs new windows. Roastty handles
        ...
        if !window.styleMask.contains(.fullScreen) {
            ...
            // Since Roastty manages tabbing manually this will never be more than one
    ...
    override func newWindowForTab(_ sender: Any?) {
        // Trigger the roastty core event logic for a new tab.
        guard let surface = self.focusedSurface?.surface else { return }
        roastty.newTab(surface: surface)
    }
    ...
    override func windowWillClose(_ notification: Notification) {
        ...
        if let focusedWindow = NSApplication.shared.keyWindow {
            ...
            if focusedWindow != window {
                ...
                // https://github.com/roastty-org/roastty/issues/2565
    ...
    @IBAction func newWindow(_ sender: Any?) {
        ...
        roastty.newWindow(surface: surface)
    }
    ...
    @IBAction func toggleRoasttyFullScreen(_ sender: Any?) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.toggleFullscreen(surface: surface)
    }

    @IBAction func toggleTerminalInspector(_ sender: Any?) {
        guard let surface = focusedSurface?.surface else { return }
        roastty.toggleTerminalInspector(surface: surface)
    }

    // MARK: - TerminalViewDelegate

    override func focusedSurfaceDidChange(to: Roastty.SurfaceView?) {
    ...
    private func syncAppearanceOnPropertyChange(_ surface: Roastty.SurfaceView?) {
    ...
    @objc private func onMoveTab(notification: SwiftUI.Notification) {
        ...
        // Get the move action
        guard let action = notification.userInfo?[Notification.Name.RoasttyMoveTabKey] as? Roastty.Action.MoveTab else { return }
    ...
    @objc private func onGotoTab(notification: SwiftUI.Notification) {
        ...
        guard let tabEnumAny = notification.userInfo?[Roastty.Notification.GotoTabKey] else { return }
        guard let tabEnum = tabEnumAny as? roastty_action_goto_tab_e else { return }
        ...
        if tabIndex <= 0 {
            ...
            if tabIndex == ROASTTY_GOTO_TAB_PREVIOUS.rawValue {
                if selectedIndex == 0 {
                    finalIndex = tabbedWindows.count - 1
                } else {
                    finalIndex = selectedIndex - 1
                }
            } else if tabIndex == ROASTTY_GOTO_TAB_NEXT.rawValue {
                if selectedIndex == tabbedWindows.count - 1 {
                    finalIndex = 0
                } else {
                    finalIndex = selectedIndex + 1
                }
            } else if tabIndex == ROASTTY_GOTO_TAB_LAST.rawValue {
                finalIndex = tabbedWindows.count - 1
            } else {
                return
            }
    ...
    @objc private func onToggleFullscreen(notification: SwiftUI.Notification) {
        ...
        if let any = notification.userInfo?[Roastty.Notification.FullscreenModeKey],
           let mode = any as? FullscreenMode {
            fullscreenMode = mode
        } else {
            Roastty.logger.warning("no fullscreen mode specified or invalid mode, doing nothing")
            return
        }
    ...
    struct DerivedConfig {
        ...
        let macosWindowButtons: Roastty.MacOSWindowButtons
        let macosTitlebarStyle: Roastty.Config.MacOSTitlebarStyle
        let maximize: Bool
