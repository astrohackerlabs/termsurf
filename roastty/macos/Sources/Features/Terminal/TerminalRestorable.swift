...
protocol TerminalRestorable: Codable {
    ...
    var baseConfig: Roastty.SurfaceConfiguration? { get }
}
...
extension TerminalRestorable {
    ...
    var baseConfig: Roastty.SurfaceConfiguration? { nil }
...
final class TerminalRestorableState: TerminalRestorable {
    ...
    var surfaceTree: SplitTree<Roastty.SurfaceView> {
    ...
    private let internalState: InternalState<Roastty.SurfaceView>
    ...
    init(from decoder: any Decoder) throws {
        self.internalState = try InternalState<Roastty.SurfaceView>(from: decoder)
    }
...
class TerminalWindowRestoration: NSObject, NSWindowRestoration {
    static func restoreWindow(
        ...
        // no matter what. Note its safe to use "roastty.config" directly here
        // because window restoration is only ever invoked on app start so we
        // don't have to deal with config reloads.
        if appDelegate.roastty.config.windowSaveState == "never" {
        ...
        // can be found for events from libroastty. This uses the low-level
        ...
        if let focusedStr = state.focusedSurface {
            var foundView: Roastty.SurfaceView?
    ...
    private static func restoreFocus(to: Roastty.SurfaceView, inWindow: NSWindow, attempts: Int = 0) {
