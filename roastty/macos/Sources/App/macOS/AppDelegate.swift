...
class AppDelegate: NSObject,
    ...
    /// Various menu items so that we can programmatically sync the keyboard shortcut with the Roastty config
    ...
    /// This is the current configuration from the Roastty configuration that we need.
    private var derivedConfig: DerivedConfig = DerivedConfig()

    /// The roastty global state. Only one per process.
    ...
    @MainActor private lazy var menuShortcutManager = Roastty.MenuShortcutManager()

    override init() {
#if DEBUG
        roastty = Roastty.App(configPath: ProcessInfo.processInfo.environment["ROASTTY_CONFIG_PATH"])
#else
        roastty = Roastty.App()
#endif
        super.init()

        roastty.delegate = self
    }
    ...
    func applicationWillFinishLaunching(_ notification: Notification) {
        #if DEBUG
        if
            let suite = UserDefaults.roasttySuite,
            let clear = ProcessInfo.processInfo.environment["ROASTTY_CLEAR_USER_DEFAULTS"],
            (clear as NSString).boolValue {
            UserDefaults.roastty.removePersistentDomain(forName: suite)
        }
        #endif
        UserDefaults.roastty.register(defaults: [
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if UserDefaults.roastty.bool(forKey: "SecureInput") != SecureInput.shared.enabled {
            toggleSecureInput(self)
        }

        // Initial config loading
        roasttyConfigDidChange(config: roastty.config)
        ...
        // This registers the Roastty => Services menu to exist.
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(roasttyNewWindow(_:)),
            name: Roastty.Notification.roasttyNewWindow,
            object: nil)
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(roasttyNewTab(_:)),
            name: Roastty.Notification.roasttyNewTab,
            object: nil)

        // Configure user notifications
        let actions = [
            UNNotificationAction(identifier: Roastty.userNotificationActionShow, title: "Show")
        ]
        ...
        center.setNotificationCategories([
            UNNotificationCategory(
                identifier: Roastty.userNotificationCategory,
                actions: actions,
                intentIdentifiers: [],
        ...
        // Observe our appearance so we can report the correct value to libroastty.
        self.appearanceObserver = NSApplication.shared.observe(
            \.effectiveAppearance,
             options: [.new, .initial]
        ) { _, change in
            guard let appearance = change.newValue else { return }
            guard let app = self.roastty.app else { return }
            ...
            roastty_app_set_color_scheme(app, scheme)
        }
    ...
    func applicationDidBecomeActive(_ notification: Notification) {
        ...
        if !applicationHasBecomeActive {
            ...
            if TerminalController.all.isEmpty && derivedConfig.initialWindow {
                ...
                _ = TerminalController.newWindow(roastty)
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        why: if let event = NSAppleEventManager.shared().currentAppleEvent {
            // If all Roastty windows are in the background (i.e. you Cmd-Q from the Cmd-Tab
        ...
        if !roastty.needsConfirmQuit { return .terminateNow }
    ...
    func application(_ sender: NSApplication, openFile filename: String) -> Bool {
        // Roastty will validate as well but we can avoid creating an entirely new
        ...
        if isDirectory.boolValue {
            ...
            config.initialInput = "\(Roastty.Shell.quote(filename)); exit\n"
        ...
        if requiresConfirm {
            ...
            let alert = NSAlert()
            alert.messageText = "Allow Roastty to execute \"\(filename)\"?"
        ...
        switch roastty.config.macosDockDropBehavior {
        case .new_tab:
        ...
        case .new_window: _ = TerminalController.newWindow(roastty, withBaseConfig: config)
        }
    ...
    private func setupSignals() {
        ...
        sigusr2.setEventHandler { [weak self] in
            ...
            Roastty.logger.info("reloading configuration in response to SIGUSR2")
            self.roastty.reloadConfig()
        }
    ...
    private func localEventKeyDown(_ event: NSEvent) -> NSEvent? {
        ...
        if let app = roastty.app, let config = roastty.config.config {
            var roasttyEvent = event.roasttyKeyEvent(ROASTTY_ACTION_PRESS)
            let match = (event.characters ?? "").withCString { ptr in
                roasttyEvent.text = ptr
                if !roastty_config_key_is_binding(config, roasttyEvent) {
                    return false
                }

                return roastty_app_key(app, roasttyEvent)
            }

            // If the key was handled by Roastty we stop the event chain. If
        ...
        // through the Roastty key mechanism.

        // Roastty must be loaded
        guard let roastty = self.roastty.app else { return event }

        // Build our event input and call roastty
        if roastty_app_key(roastty, event.roasttyKeyEvent(ROASTTY_ACTION_PRESS)) {
            // The key was used so we want to stop it from going to our Mac app
            Roastty.logger.debug("local key event handled event=\(event)")
            return nil
        }
    ...
    @objc private func roasttyConfigDidChange(_ notification: Notification) {
        ...
        roasttyConfigDidChange(config: config)
    }
    ...
    @objc private func roasttyBellDidRing(_ notification: Notification) {
        ...
        if roastty.config.bellFeatures.contains(.audio) {
    ...
    @objc private func roasttyNewWindow(_ notification: Notification) {
        ...
        _ = TerminalController.newWindow(roastty, withBaseConfig: config)
    }

    @objc private func roasttyNewTab(_ notification: Notification) {
        ...
        _ = TerminalController.newTab(roastty, from: window, withBaseConfig: config)
    }
    ...
    private func setDockBadge() {
        ...
        let wantsBadge = roastty.config.bellFeatures.contains(.attention) && bellCount > 0
    ...
    private func roasttyConfigDidChange(config: Roastty.Config) {
        ...
        case "never": UserDefaults.roastty.setValue(false, forKey: "NSQuitAlwaysKeepsWindows")
        case "always": UserDefaults.roastty.setValue(true, forKey: "NSQuitAlwaysKeepsWindows")
        case "default": fallthrough
        default: UserDefaults.roastty.removeObject(forKey: "NSQuitAlwaysKeepsWindows")
        }
        ...
        if Bundle.main.infoDictionary?["SUEnableAutomaticChecks"] as? Bool == false {
            ...
            /*
             ...
             delete `SUEnableAutomaticChecks` in Roastty-Info.plist.
        ...
        // events that we care about in Roastty.
        if roastty_app_has_global_keybinds(roastty.app!) {
    ...
    private func syncAppearance(config: Roastty.Config) {
        NSApplication.shared.appearance = .init(roasttyConfig: config)
    }

    private func updateAppIcon(from config: Roastty.Config) {
        Task.detached {
    ...
    func application(_ app: NSApplication, willEncodeRestorableState coder: NSCoder) {
        guard roastty.config.windowSaveState != "never" else { return }
    ...
    func application(_ app: NSApplication, didDecodeRestorableState coder: NSCoder) {
        ...
        if roastty.config.windowSaveState != "never",
    ...
    func userNotificationCenter(
        ...
        roastty.handleUserNotification(response: didReceive)
    ...
    func setSecureInput(_ mode: Roastty.SetSecureInput) {
        ...
        UserDefaults.roastty.set(input.global, forKey: "SecureInput")
    }
    ...
    @IBAction func showHelp(_ sender: Any) {
        guard let url = URL(string: "https://roastty.org/docs") else { return }
    ...
    /// Toggles visibility of all Ghosty Terminal windows. When hidden, activates Roastty as the frontmost application
    @IBAction func toggleVisibility(_ sender: Any) {
        // If we have focus, then we hide all windows.
        if NSApp.isActive {
            // Toggle visibility doesn't do anything if the focused window is native
            // fullscreen. This is only relevant if Roastty is active.
...
extension AppDelegate {
    ...
    /// Sync all of our menu item keyboard shortcuts with the Roastty configuration.
    @MainActor private func syncMenuShortcuts(_ config: Roastty.Config) {
        guard roastty.readiness == .ready else { return }
        ...
        // global fullscreen keyboard shortcut. The shortcut in the Roastty config will continue
    ...
    @MainActor private func syncMenuShortcut(_ config: Roastty.Config, action: String, menuItem: NSMenuItem?) {
        menuShortcutManager.syncMenuShortcut(config, action: action, menuItem: menuItem)
    }

    @MainActor func performRoasttyBindingMenuKeyEquivalent(with event: NSEvent) -> Bool {
        menuShortcutManager.performRoasttyBindingMenuKeyEquivalent(with: event)
    }
}
    ...
    @IBAction func setAsDefaultTerminal(_ sender: NSMenuItem) {
        NSWorkspace.shared.setDefaultApplication(at: Bundle.main.bundleURL, toOpen: .unixExecutable) { error in
            ...
            Task { @MainActor in
                ...
                Roastty could not be set as the default terminal application.
