...
#if !DOCK_TILE_PLUGIN
extension OSColor {
    /// Create a color from a Roastty color.
    convenience init(roastty: roastty_config_color_s) {
        let red = Double(roastty.r) / 255
        let green = Double(roastty.g) / 255
        let blue = Double(roastty.b) / 255
