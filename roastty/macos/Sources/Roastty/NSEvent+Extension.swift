...
extension NSEvent {
    /// Create a Roastty key event for a given keyboard action.
    ///
    ...
    func roasttyKeyEvent(
        _ action: roastty_input_action_e,
        translationMods: NSEvent.ModifierFlags? = nil
    ) -> roastty_input_key_s {
        var key_ev: roastty_input_key_s = .init()
        ...
        key_ev.mods = Roastty.roasttyMods(modifierFlags)
        key_ev.consumed_mods = Roastty.roasttyMods(
    ...
    /// Returns the text to set for a key event for Roastty.
    ///
    /// This namely contains logic to avoid control characters, since we handle control character
    /// mapping manually within Roastty.
    var roasttyCharacters: String? {
        ...
        if characters.count == 1,
           let scalar = characters.unicodeScalars.first {
            ...
            // encoding directly within Roastty's KeyEncoder.
            if scalar.value < 0x20 {
                return self.characters(byApplyingModifiers: modifierFlags.subtracting(.control))
            }

            // If we have a single value in the PUA, then it's a function key and
            // we don't want to send PUA ranges down to Roastty.
