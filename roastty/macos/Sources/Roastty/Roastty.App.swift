...
protocol RoasttyAppDelegate: AnyObject {
    #if os(macOS)
    /// Called when a callback needs access to a specific surface. This should return nil
    /// when the surface is no longer valid.
    func findSurface(forUUID uuid: UUID) -> Roastty.SurfaceView?
    #endif
}

extension Roastty {
    // IMPORTANT: THIS IS NOT DONE.
    // This is a refactor/redo of Roastty.AppState so that it supports both macOS and iOS
    class App: ObservableObject {
        ...
        /// Optional delegate
        weak var delegate: RoasttyAppDelegate?
        ...
        /// The roastty app instance. We only have one of these for the entire app, although I guess
        /// in theory you can have multiple... I don't know why you would...
        @Published var app: roastty_app_t? {
            didSet {
                guard let old = oldValue else { return }
                roastty_app_free(old)
            }
        }
        ...
        var needsConfirmQuit: Bool {
            ...
            return roastty_app_needs_confirm_quit(app)
        }
        ...
        init(configPath: String? = nil) {
            ...
            // Create our "runtime" config. The "runtime" is the configuration that roastty
            // uses to interface with the application runtime environment.
            var runtime_cfg = roastty_runtime_config_s(
            ...
            // Create the roastty app.
            guard let app = roastty_app_new(&runtime_cfg, config.config) else {
                logger.critical("roastty_app_new failed")
                readiness = .error
                return
            }
            self.app = app
            ...
            roastty_app_set_focus(app, NSApp.isActive)
        ...
        private static func openConfig(_ app: roastty_app_t) {
            guard let app_ud = roastty_app_userdata(app) else { return }
            let app = Unmanaged<App>.fromOpaque(app_ud).takeUnretainedValue()
            app.openConfig()
        }

        func openConfig() {
            let str = configPath ?? Roastty.AllocatedString(roastty_config_open_path()).string
            guard !str.isEmpty else { return }
            #if os(macOS)
            let fileURL = URL(fileURLWithPath: str).absoluteString
            var action = roastty_action_open_url_s()
            action.kind = ROASTTY_ACTION_OPEN_URL_KIND_TEXT
        ...
        func reloadConfig(soft: Bool = false) {
            ...
            if soft {
                roastty_app_update_config(app, config.config!)
                return
            }
            ...
            guard newConfig.loaded else {
                Roastty.logger.warning("failed to reload configuration")
                return
            }

            roastty_app_update_config(app, newConfig.config!)
            /// applied config will be updated in ``Self.configChange(_:target:v:)``
        }

        func reloadConfig(surface: roastty_surface_t, soft: Bool = false) {
            // Soft updates just call with our existing config
            if soft {
                roastty_surface_update_config(surface, config.config!)
                return
            }
            ...
            roastty_surface_update_config(surface, newConfig.config!)
        }
        ...
        func requestClose(surface: roastty_surface_t) {
            roastty_surface_request_close(surface)
        }

        func newTab(surface: roastty_surface_t) {
            let action = "new_tab"
            if !roastty_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8))) {
                logger.warning("action failed action=\(action)")
            }
        }

        func newWindow(surface: roastty_surface_t) {
        ...
        func split(surface: roastty_surface_t, direction: roastty_action_split_direction_e) {
            roastty_surface_split(surface, direction)
        }

        func splitMoveFocus(surface: roastty_surface_t, direction: SplitFocusDirection) {
            roastty_surface_split_focus(surface, direction.toNative())
        }

        func splitResize(surface: roastty_surface_t, direction: SplitResizeDirection, amount: UInt16) {
            roastty_surface_split_resize(surface, direction.toNative(), amount)
        }

        func splitEqualize(surface: roastty_surface_t) {
            roastty_surface_split_equalize(surface)
        }

        func splitToggleZoom(surface: roastty_surface_t) {
        ...
        func toggleFullscreen(surface: roastty_surface_t) {
        ...
        func changeFontSize(surface: roastty_surface_t, _ change: FontSizeModification) {
            let action: String
            switch change {
        ...
        func toggleTerminalInspector(surface: roastty_surface_t) {
        ...
        func resetTerminal(surface: roastty_surface_t) {
            let action = "reset"
            if !roastty_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8))) {
                logger.warning("action failed action=\(action)")
            }
        ...
        #if os(iOS)
        // MARK: Roastty Callbacks (iOS)

        static func wakeup(_ userdata: UnsafeMutableRawPointer?) {}
        static func action(_ app: roastty_app_t, target: roastty_target_s, action: roastty_action_s) -> Bool { return false }
        static func readClipboard(
            _ userdata: UnsafeMutableRawPointer?,
            location: roastty_clipboard_e,
        ...
        static func confirmReadClipboard(
            ...
            state: UnsafeMutableRawPointer?,
            request: roastty_clipboard_request_e
        ) {}

        static func writeClipboard(
            _ userdata: UnsafeMutableRawPointer?,
            location: roastty_clipboard_e,
            content: UnsafePointer<roastty_clipboard_content_s>?,
            len: Int,
            confirm: Bool
        ) {}
        ...
        // Called when the selected keyboard changes. We have to notify Roastty so that
        ...
        // MARK: Roastty Callbacks (macOS)

        static func closeSurface(_ userdata: UnsafeMutableRawPointer?, processAlive: Bool) {
            let surface = self.surfaceUserdata(from: userdata)
            NotificationCenter.default.post(name: Notification.roasttyCloseSurface, object: surface, userInfo: [
        ...
        static func readClipboard(
            ...
            // Get our pasteboard
            guard let pasteboard = NSPasteboard.roastty(location) else { return false }
        ...
        static func confirmReadClipboard(
            ...
            guard let request = Roastty.ClipboardRequest.from(request: request) else { return }
        ...
        static func completeClipboardRequest(
            _ surface: roastty_surface_t,
            data: String,
            state: UnsafeMutableRawPointer?,
            confirmed: Bool = false
        ) {
            data.withCString { ptr in
                roastty_surface_complete_clipboard_request(surface, ptr, state, confirmed)
            }
        }
        ...
        static func writeClipboard(
            ...
            let surface = self.surfaceUserdata(from: userdata)
            guard let pasteboard = NSPasteboard.roastty(location) else { return }
            guard let content = content, len > 0 else { return }

            // Convert the C array to Swift array
            let contentArray = (0..<len).compactMap { i in
                Roastty.ClipboardContent.from(content: content[i])
            }
            ...
            NotificationCenter.default.post(
                ...
                userInfo: [
                    ...
                    Notification.ConfirmClipboardRequestKey: Roastty.ClipboardRequest.osc_52_write(pasteboard),
                ]
            )
        }
        ...
        /// Determine if a given notification should be presented to the user when Roastty is running in the foreground.
        ...
        /// Returns the RoasttyState from the given userdata value.
        static private func appState(fromView view: SurfaceView) -> App? {
            guard let surface = view.surface else { return nil }
            guard let app = roastty_surface_app(surface) else { return nil }
            guard let app_ud = roastty_app_userdata(app) else { return nil }
        ...
        private static func quit(_ app: roastty_app_t) {
        ...
        private static func openURL(
            _ v: roastty_action_open_url_s
        ) -> Bool {
            let action = Roastty.Action.OpenURL(c: v)
            ...
            // See: https://github.com/roastty-org/roastty/issues/8763
            let url: URL
        ...
        private static func redo(_ app: roastty_app_t, target: roastty_target_s) -> Bool {
            let undoManager: UndoManager?
            switch target.tag {
            case ROASTTY_TARGET_APP:
                undoManager = (NSApp.delegate as? AppDelegate)?.undoManager
        ...
        private static func newWindow(_ app: roastty_app_t, target: roastty_target_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                NotificationCenter.default.post(
                    name: Notification.roasttyNewWindow,
                    object: nil,
                    userInfo: [:]
                )
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    ...
                    object: surfaceView,
                    userInfo: [
                        Notification.NewSurfaceConfigKey: SurfaceConfiguration(from: roastty_surface_inherited_config(surface, ROASTTY_SURFACE_CONTEXT_WINDOW)),
                    ]
                )
        ...
        private static func newTab(_ app: roastty_app_t, target: roastty_target_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                NotificationCenter.default.post(
                    name: Notification.roasttyNewTab,
                    object: nil,
                    userInfo: [:]
                )
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    ...
                    object: surfaceView,
                    userInfo: [
                        Notification.NewSurfaceConfigKey: SurfaceConfiguration(from: roastty_surface_inherited_config(surface, ROASTTY_SURFACE_CONTEXT_TAB)),
                    ]
                )
        ...
        private static func newSplit(
            _ app: roastty_app_t,
            target: roastty_target_s,
            direction: roastty_action_split_direction_e) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                // New split does nothing with an app target
                Roastty.logger.warning("new split does nothing with an app target")
                return
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    name: Notification.roasttyNewSplit,
                    object: surfaceView,
                    userInfo: [
                        "direction": direction,
                        Notification.NewSurfaceConfigKey: SurfaceConfiguration(from: roastty_surface_inherited_config(surface, ROASTTY_SURFACE_CONTEXT_SPLIT)),
                    ]
                )
        ...
        private static func presentTerminal(
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    name: Notification.roasttyPresentTerminal,
                    object: surfaceView
                )
                return true
        ...
        private static func closeTab(_ app: roastty_app_t, target: roastty_target_s, mode: roastty_action_close_tab_mode_e) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("close tabs does nothing with an app target")
                return
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                switch mode {
                case ROASTTY_ACTION_CLOSE_TAB_MODE_THIS:
                    NotificationCenter.default.post(
                        name: .roasttyCloseTab,
                        object: surfaceView
                    )
                    return

                case ROASTTY_ACTION_CLOSE_TAB_MODE_OTHER:
                    NotificationCenter.default.post(
                        name: .roasttyCloseOtherTabs,
                        object: surfaceView
                    )
                    return

                case ROASTTY_ACTION_CLOSE_TAB_MODE_RIGHT:
                    NotificationCenter.default.post(
                        name: .roasttyCloseTabsOnTheRight,
                        object: surfaceView
                    )
                    return
        ...
        private static func closeWindow(_ app: roastty_app_t, target: roastty_target_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("close window does nothing with an app target")
                return
        ...
        private static func closeAllWindows(_ app: roastty_app_t, target: roastty_target_s) {
        ...
        private static func toggleFullscreen(
            _ app: roastty_app_t,
            target: roastty_target_s,
            mode raw: roastty_action_fullscreen_e) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("toggle fullscreen does nothing with an app target")
                return

            case ROASTTY_TARGET_SURFACE:
                guard let surface = target.target.surface else { return }
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                guard let mode = FullscreenMode.from(roastty: raw) else {
                    Roastty.logger.warning("unknown fullscreen mode raw=\(raw.rawValue)")
                    return
                }
                NotificationCenter.default.post(
                    name: Notification.roasttyToggleFullscreen,
                    object: surfaceView,
                    userInfo: [
        ...
        private static func toggleCommandPalette(
            ...
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("toggle command palette does nothing with an app target")
                return
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                NotificationCenter.default.post(
                    name: .roasttyCommandPaletteDidToggle,
                    object: surfaceView
                )
        ...
        private static func toggleMaximize(
            ...
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("toggle maximize does nothing with an app target")
                return
        ...
        private static func ringBell(
            ...
            case ROASTTY_TARGET_APP:
                ...
                Roastty.logger.warning("ring bell does nothing with an app target")
                return
        ...
        private static func setReadonly(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_readonly_e) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("set readonly does nothing with an app target")
                return
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                NotificationCenter.default.post(
                    name: .roasttyDidChangeReadonly,
                    object: surfaceView,
                    userInfo: [
                        SwiftUI.Notification.Name.ReadonlyKey: v == ROASTTY_READONLY_ON,
                    ]
                )
        ...
        private static func moveTab(
            _ app: roastty_app_t,
            target: roastty_target_s,
            move: roastty_action_move_tab_s) -> Bool {
                switch target.tag {
                case ROASTTY_TARGET_APP:
                    Roastty.logger.warning("move tab does nothing with an app target")
                    return false
                ...
                case ROASTTY_TARGET_SURFACE:
                    ...
                    NotificationCenter.default.post(
                        ...
                        object: surfaceView,
                        userInfo: [
                            SwiftUI.Notification.Name.RoasttyMoveTabKey: Action.MoveTab(c: move),
                        ]
                    )
        ...
        private static func gotoTab(
            _ app: roastty_app_t,
            target: roastty_target_s,
            tab: roastty_action_goto_tab_e) -> Bool {
                switch target.tag {
                case ROASTTY_TARGET_APP:
                    Roastty.logger.warning("goto tab does nothing with an app target")
                    return false
                ...
                case ROASTTY_TARGET_SURFACE:
                    ...
                    NotificationCenter.default.post(
                        name: Notification.roasttyGotoTab,
                        object: surfaceView,
                        userInfo: [
        ...
        private static func gotoSplit(
            _ app: roastty_app_t,
            target: roastty_target_s,
            direction: roastty_action_goto_split_e) -> Bool {
                switch target.tag {
                case ROASTTY_TARGET_APP:
                    Roastty.logger.warning("goto split does nothing with an app target")
                    return false
                ...
                case ROASTTY_TARGET_SURFACE:
                    ...
                    let focusDirection: SplitTree<Roastty.SurfaceView>.FocusDirection = splitDirection.toSplitTreeFocusDirection()
                    ...
                    NotificationCenter.default.post(
                        name: Notification.roasttyFocusSplit,
                        object: surfaceView,
                        userInfo: [
        ...
        private static func gotoWindow(
            _ app: roastty_app_t,
            target: roastty_target_s,
            direction: roastty_action_goto_window_e
        ) -> Bool {
            ...
            let step: Int
            switch direction {
            case ROASTTY_GOTO_WINDOW_NEXT:
                step = 1
            case ROASTTY_GOTO_WINDOW_PREVIOUS:
                step = -1
            default:
                return false
            }
        ...
        private static func resizeSplit(
            _ app: roastty_app_t,
            target: roastty_target_s,
            resize: roastty_action_resize_split_s) -> Bool {
                switch target.tag {
                case ROASTTY_TARGET_APP:
                    Roastty.logger.warning("resize split does nothing with an app target")
                    return false
        ...
        private static func equalizeSplits(
            ...
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("equalize splits does nothing with an app target")
                return
        ...
        private static func toggleSplitZoom(
            _ app: roastty_app_t,
            target: roastty_target_s) -> Bool {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("toggle split zoom does nothing with an app target")
                return false
        ...
        private static func controlInspector(
            _ app: roastty_app_t,
            target: roastty_target_s,
            mode: roastty_action_inspector_e) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("toggle inspector does nothing with an app target")
                return
        ...
        private static func showDesktopNotification(
            _ app: roastty_app_t,
            target: roastty_target_s,
            n: roastty_action_desktop_notification_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("desktop notification does nothing with an app target")
                return
        ...
        private static func commandFinished(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_command_finished_s
        ) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("command finished does nothing with an app target")
                return
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                guard let config = (NSApplication.shared.delegate as? AppDelegate)?.roastty.config else { return }
        ...
        private static func toggleFloatWindow(
            _ app: roastty_app_t,
            target: roastty_target_s,
            mode mode_raw: roastty_action_float_window_e
        ) {
            guard let mode = SetFloatWIndow.from(mode_raw) else { return }

            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("toggle float window does nothing with an app target")
                return
        ...
        private static func toggleBackgroundOpacity(
            ...
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("toggle background opacity does nothing with an app target")
                return
        ...
        private static func toggleSecureInput(
            _ app: roastty_app_t,
            target: roastty_target_s,
            mode mode_raw: roastty_action_secure_input_e
        ) {
        ...
        private static func setTitle(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_set_title_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("set title does nothing with an app target")
                return
        ...
        private static func setTabTitle(
            ...
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("set tab title does nothing with an app target")
                return false
        ...
        private static func showChildExited(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_surface_message_childexited_s,
        ) -> Bool {
            switch target.tag {
            case ROASTTY_TARGET_SURFACE:
                guard let surface = target.target.surface else { return false }
                ...
                guard let config = (NSApplication.shared.delegate as? AppDelegate)?.roastty.config else { return false }
        ...
        private static func promptTitle(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_prompt_title_e) -> Bool {
            let promptTitle = Action.PromptTitle(v)
            switch promptTitle {
            case .surface:
                switch target.tag {
                case ROASTTY_TARGET_APP:
                    Roastty.logger.warning("set title prompt does nothing with an app target")
                    return false
        ...
        private static func pwdChanged(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_pwd_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("pwd change does nothing with an app target")
                return
        ...
        private static func setMouseShape(
            _ app: roastty_app_t,
            target: roastty_target_s,
            shape: roastty_action_mouse_shape_e) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("set mouse shapes nothing with an app target")
                return
        ...
        private static func setMouseVisibility(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_mouse_visibility_e) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("set mouse shapes nothing with an app target")
                return
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                switch v {
                case ROASTTY_MOUSE_VISIBLE:
                    surfaceView.setCursorVisibility(true)

                case ROASTTY_MOUSE_HIDDEN:
        ...
        private static func setMouseOverLink(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_mouse_over_link_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("mouse over link does nothing with an app target")
                return
        ...
        private static func setInitialSize(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_initial_size_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("initial size does nothing with an app target")
                return
        ...
        private static func resetWindowSize(
            ...
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("reset window size does nothing with an app target")
                return
        ...
        private static func rendererHealth(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_renderer_health_e) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("mouse over link does nothing with an app target")
                return
        ...
        private static func keySequence(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_key_sequence_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("key sequence does nothing with an app target")
                return
        ...
        private static func keyTable(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_key_table_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("key table does nothing with an app target")
                return

            case ROASTTY_TARGET_SURFACE:
                guard let surface = target.target.surface else { return }
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                guard let action = Roastty.Action.KeyTable(c: v) else { return }
        ...
        private static func progressReport(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_progress_report_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("progress report does nothing with an app target")
                return
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                guard config.progressStyle else {
                    Roastty.logger.debug("progress_report action blocked by config")
                ...
                let progressReport = Roastty.Action.ProgressReport(c: v)
                DispatchQueue.main.async {
        ...
        private static func scrollbar(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_scrollbar_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("scrollbar does nothing with an app target")
                return
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                let scrollbar = Roastty.Action.Scrollbar(c: v)
                NotificationCenter.default.post(
                    name: .roasttyDidUpdateScrollbar,
                    object: surfaceView,
                    userInfo: [
        ...
        private static func startSearch(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_start_search_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("start_search does nothing with an app target")
                return
            ...
            case ROASTTY_TARGET_SURFACE:
                ...
                let startSearch = Roastty.Action.StartSearch(c: v)
                DispatchQueue.main.async {
                    if let searchState = surfaceView.searchState {
                        ...
                        surfaceView.searchState = Roastty.SurfaceView.SearchState(from: startSearch)
                    }

                    NotificationCenter.default.post(name: .roasttySearchFocus, object: surfaceView)
                }
        ...
        private static func endSearch(
            ...
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("end_search does nothing with an app target")
                return false
        ...
        private static func searchTotal(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_search_total_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("search_total does nothing with an app target")
                return
        ...
        private static func searchSelected(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_search_selected_s) {
            switch target.tag {
            case ROASTTY_TARGET_APP:
                Roastty.logger.warning("search_selected does nothing with an app target")
                return
        ...
        private static func configReload(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_reload_config_s) {
            logger.info("config reload notification")

            guard let app_ud = roastty_app_userdata(app) else { return }
            let roastty = Unmanaged<App>.fromOpaque(app_ud).takeUnretainedValue()

            switch target.tag {
            case ROASTTY_TARGET_APP:
                roastty.reloadConfig(soft: v.soft)
                return

            case ROASTTY_TARGET_SURFACE:
                guard let surface = target.target.surface else { return }
                roastty.reloadConfig(surface: surface, soft: v.soft)
        ...
        private static func configChange(
            _ app: roastty_app_t,
            target: roastty_target_s,
            v: roastty_action_config_change_s) {
                ...
                case ROASTTY_TARGET_APP:
                    ...
                    NotificationCenter.default.post(
                        name: .roasttyConfigDidChange,
                        object: nil,
                        userInfo: [
                            SwiftUI.Notification.Name.RoasttyConfigChangeKey: config,
                        ]
                    )
        ...
        private static func colorChange(
            _ app: roastty_app_t,
            target: roastty_target_s,
            change: roastty_action_color_change_s) {
                switch target.tag {
                case ROASTTY_TARGET_APP:
                    Roastty.logger.warning("color change does nothing with an app target")
                    return
                ...
                case ROASTTY_TARGET_SURFACE:
                    ...
                    guard let surfaceView = self.surfaceView(from: surface) else { return }
                    NotificationCenter.default.post(
                        name: .roasttyColorDidChange,
                        object: surfaceView,
                        userInfo: [
                            SwiftUI.Notification.Name.RoasttyColorChangeKey: Action.ColorChange(c: change)
                        ]
                    )
        ...
        func handleUserNotification(response: UNNotificationResponse) {
            ...
            case UNNotificationDefaultActionIdentifier, Roastty.userNotificationActionShow:
