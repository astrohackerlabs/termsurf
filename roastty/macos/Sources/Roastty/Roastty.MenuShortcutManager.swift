...
extension Roastty {
    /// The manager that's responsible for updating shortcuts of Roastty's app menu
    @MainActor
    class MenuShortcutManager {

        /// Roastty menu items indexed by their normalized shortcut. This avoids traversing
        ...
        /// action string used for the Roastty configuration.
        func syncMenuShortcut(_ config: Roastty.Config, action: String?, menuItem: NSMenuItem?) {
        ...
        /// Roastty keybind actions. This is important because it lets our surface dispatch
        /// bindings through the menu so they flash but also lets our surface override macOS built-ins
        /// like Cmd+H.
        func performRoasttyBindingMenuKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            // If we don't have an entry for this key combo, no Roastty-owned
...
private extension Roastty.MenuShortcutManager {
    /// Syncs a single menu shortcut for the given action. The action string is the same
    /// action string used for the Roastty configuration.
    ///
    /// - Returns: Whether the menu item is updated and saved in ``menuItemsByShortcut``
    func updateMenuShortcut(_ config: Roastty.Config, action: String?, menuItem menu: NSMenuItem) -> Bool {
        guard
            let action,
...
extension Roastty.MenuShortcutManager {
    ...
    struct MenuShortcutKey: Hashable {
        ...
        init?(_ shortcut: KeyboardShortcut) {
            // Roastty configured shortcuts are already normalized
            // in `Roastty.keyboardShortcut(for:)`, see also gh-#12039
