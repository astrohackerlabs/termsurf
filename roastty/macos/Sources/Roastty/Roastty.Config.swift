...
extension Roastty {
    /// Maps to a `roastty_config_t` and the various operations on that.
    class Config: ObservableObject {
        // The underlying C pointer to the Roastty config structure. This
        // should never be accessed directly. Any operations on this should
        // be called from the functions on this or another class.
        private(set) var config: roastty_config_t? {
            didSet {
                // Free the old value whenever we change
                guard let old = oldValue else { return }
                roastty_config_free(old)
            }
        }
        ...
        var errors: [String] {
            ...
            var diags: [String] = []
            let diagsCount = roastty_config_diagnostics_count(cfg)
            for i in 0..<diagsCount {
                let diag = roastty_config_get_diagnostic(cfg, UInt32(i))
        ...
        convenience init(clone config: roastty_config_t) {
            self.init(config: roastty_config_clone(config))
        }

        func clone(config: roastty_config_t) {
            self.config = config
        }
        ...
        static func loadConfig(at path: String?, finalize: Bool) -> roastty_config_t? {
            // Initialize the global configuration.
            guard let cfg = roastty_config_new() else {
                logger.critical("roastty_config_new failed")
                return nil
            }
...
#if os(macOS)
            if let path {
                roastty_config_load_file(cfg, path)
            } else {
                roastty_config_load_default_files(cfg)
            }

            // We only load CLI args when not running in Xcode because in Xcode we
            // pass some special parameters to control the debugger.
            if !isRunningInXcode() {
                roastty_config_load_cli_args(cfg)
            }

            roastty_config_load_recursive_files(cfg)
#endif
        ...
        /// in the Roastty configuration. For example `keybind = cmd+q=quit` in Roastty
        ...
        func keyboardShortcut(for action: String) -> KeyboardShortcut? {
            ...
            let trigger = roastty_config_trigger(cfg, action, UInt(action.lengthOfBytes(using: .utf8)))
            return Roastty.keyboardShortcut(for: trigger)
        }
#endif

        // MARK: - Configuration Values

        /// For all of the configuration values below, see the associated Roastty documentation for
        ...
        var bellFeatures: BellFeatures {
            ...
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .init() }
            return .init(rawValue: v)
        }

        var bellAudioPath: ConfigPath? {
            guard let config = self.config else { return nil }
            var v = roastty_config_path_s()
        ...
        var bellAudioVolume: Float {
            ...
            _ = roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8)))
            return Float(v)
        }
        ...
        var notifyOnCommandFinish: NotifyOnCommandFinish {
            ...
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .never }
        ...
        var notifyOnCommandFinishAction: NotifyOnCommandFinishAction {
            ...
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return defaultValue }
            return .init(rawValue: v)
        }
        ...
        var splitPreserveZoom: SplitPreserveZoom {
            ...
            guard roastty_config_get(config, &v, key, UInt(key.count)) else { return .init() }
            return .init(rawValue: v)
        }
        ...
        var windowSaveState: String {
            ...
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return "" }
        ...
        var windowPositionX: Int16? {
            ...
            return roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) ? v : nil
        }
        ...
        var windowFullscreen: Bool {
            ...
            var v: UnsafePointer<Int8>?
            let key = "fullscreen"
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return false }
        ...
        var macosCustomIcon: String {
            #if os(macOS)
            let defaultValue = NSString("~/.config/roastty/Roastty.icns").expandingTildeInPath
            guard let config = self.config else { return defaultValue }
            var v: UnsafePointer<Int8>?
        ...
        var backgroundColor: Color {
            var color: roastty_config_color_s = .init()
            let bg_key = "background"
            if !roastty_config_get(config, &color, bg_key, UInt(bg_key.lengthOfBytes(using: .utf8))) {
#if os(macOS)
...
#error("unsupported")
        ...
        var unfocusedSplitOpacity: Double {
            ...
            _ = roastty_config_get(config, &opacity, key, UInt(key.lengthOfBytes(using: .utf8)))
            return 1 - opacity
        }
        ...
        var unfocusedSplitFill: Color {
            ...
            if !roastty_config_get(config, &color, key, UInt(key.lengthOfBytes(using: .utf8))) {
                let bg_key = "background"
                _ = roastty_config_get(config, &color, bg_key, UInt(bg_key.lengthOfBytes(using: .utf8)))
            }
        ...
        var quickTerminalPosition: QuickTerminalPosition {
            ...
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .top }
        ...
        var quickTerminalScreen: QuickTerminalScreen {
            ...
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .main }
            guard let ptr = v else { return .main }
            let str = String(cString: ptr)
            return QuickTerminalScreen(fromRoasttyConfig: str) ?? .main
        }
        ...
        var quickTerminalSpaceBehavior: QuickTerminalSpaceBehavior {
            ...
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .move }
            guard let ptr = v else { return .move }
            let str = String(cString: ptr)
            return QuickTerminalSpaceBehavior(fromRoasttyConfig: str) ?? .move
        }

        var quickTerminalSize: QuickTerminalSize {
            guard let config = self.config else { return QuickTerminalSize() }
            var v = roastty_config_quick_terminal_size_s()
            let key = "quick-terminal-size"
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return QuickTerminalSize() }
        ...
        var resizeOverlay: ResizeOverlay {
            ...
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .after_first }
        ...
        var commandPaletteEntries: [Roastty.Command] {
            guard let config = self.config else { return [] }
            var v: roastty_config_command_list_s = .init()
            let key = "command-palette-entry"
            guard roastty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return [] }
            guard v.len > 0 else { return [] }
            let buffer = UnsafeBufferPointer(start: v.commands, count: v.len)
            return buffer.map { Roastty.Command(cValue: $0) }
        }
