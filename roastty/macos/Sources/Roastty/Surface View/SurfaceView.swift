...
extension Roastty {
    ...
    struct Terminal: View {
        @EnvironmentObject private var roastty: Roastty.App

        var body: some View {
            if let app = self.roastty.app {
    ...
    /// Yields a SurfaceView for a roastty app that can then be used however you want.
    struct SurfaceForApp<Content: View>: View {
        let content: ((SurfaceView) -> Content)

        @StateObject private var surfaceView: SurfaceView

        init(_ app: roastty_app_t, @ViewBuilder content: @escaping ((SurfaceView) -> Content)) {
    ...
    struct SurfaceWrapper: View {
        ...
        @EnvironmentObject private var roastty: Roastty.App
        @Environment(\.roasttyLastFocusedSurface) private var lastFocusedSurface
            ...
            ZStack {
                ...
                GeometryReader { geo in
                    ...
                    SurfaceRepresentable(view: surfaceView, size: geo.size)
                        ...
                        .focusedValue(\.roasttySurfacePwd, surfaceView.pwd)
                        .focusedValue(\.roasttySurfaceView, surfaceView)
                        .focusedValue(\.roasttySurfaceCellSize, surfaceView.cellSize)
                    #if canImport(AppKit)
                    ...
                    if let surfaceSize = surfaceView.surfaceSize {
                        SurfaceResizeOverlay(
                            geoSize: geo.size,
                            size: surfaceSize,
                            overlay: roastty.config.resizeOverlay,
                            position: roastty.config.resizeOverlayPosition,
                            duration: roastty.config.resizeOverlayDuration,
                            focusInstant: surfaceView.focusInstant)

                    }
                }
                .roasttySurfaceView(surfaceView)
                ...
                if roastty.config.secureInputIndication &&
                    secureInput.enabled &&
                    surfaceFocus &&
                    windowFocus {
                    SecureInputOverlay()
                ...
                if roastty.config.bellFeatures.contains(.border) {
                ...
                if !surfaceView.healthy {
                    Rectangle().fill(roastty.config.backgroundColor)
                ...
                if isSplit && !isFocusedSurface {
                    let overlayOpacity = roastty.config.unfocusedSplitOpacity
                    if overlayOpacity > 0 {
                        Rectangle()
                            .fill(roastty.config.unfocusedSplitFill)
                            .allowsHitTesting(false)
    ...
    struct SurfaceResizeOverlay: View {
        let geoSize: CGSize
        let size: roastty_surface_size_s
        let overlay: Roastty.Config.ResizeOverlay
        let position: Roastty.Config.ResizeOverlayPosition
        let duration: UInt
    ...
    struct SurfaceSearchOverlay: View {
        ...
        var body: some View {
            GeometryReader { geo in
                HStack(spacing: 4) {
                    ...
                    .onExitCommand {
                        if searchState.needle.isEmpty {
                            ...
                            Roastty.moveFocus(to: surfaceView)
                        }
                    }
#endif
                ...
                .onReceive(NotificationCenter.default.publisher(for: .roasttySearchFocus)) { notification in
    ...
    /// A surface is terminology in Roastty for a terminal surface, or a place where a terminal is actually drawn
    ...
    /// libroastty, usually from the Roastty configuration.
    struct SurfaceConfiguration {
        ...
        var context: roastty_surface_context_e = ROASTTY_SURFACE_CONTEXT_WINDOW

        init() {}

        init(from config: roastty_surface_config_s) {
        ...
        /// Provides a C-compatible roastty configuration within a closure. The configuration
        /// and all its string pointers are only valid within the closure.
        func withCValue<T>(view: SurfaceView, _ body: (inout roastty_surface_config_s) throws -> T) rethrows -> T {
            var config = roastty_surface_config_new()
            config.userdata = Unmanaged.passUnretained(view).toOpaque()
#if os(macOS)
            config.platform_tag = ROASTTY_PLATFORM_MACOS
            config.platform = roastty_platform_u(macos: roastty_platform_macos_s(
                nsview: Unmanaged.passUnretained(view).toOpaque()
            ))
            config.scale_factor = NSScreen.main!.backingScaleFactor
#elseif os(iOS)
            config.platform_tag = ROASTTY_PLATFORM_IOS
            config.platform = roastty_platform_u(ios: roastty_platform_ios_s(
            ...
            // libroastty APIs when a UIView is attached to a window/scene. TODO.
                    ...
                    return try initialInput.withCString { cInput in
                        ...
                        return try keys.withCStrings { keyCStrings in
                            return try values.withCStrings { valueCStrings in
                                // Create array of roastty_env_var_s
                                var envVars = [roastty_env_var_s]()
                                envVars.reserveCapacity(environmentVariables.count)
                                for i in 0..<environmentVariables.count {
                                    envVars.append(roastty_env_var_s(
                                        key: keyCStrings[i],
...
private struct RoasttySurfaceViewKey: EnvironmentKey {
    static let defaultValue: Roastty.SurfaceView? = nil
}

private struct RoasttyLastFocusedSurfaceKey: EnvironmentKey {
    ...
    static let defaultValue: Weak<Roastty.SurfaceView>? = nil
}

extension EnvironmentValues {
    var roasttySurfaceView: Roastty.SurfaceView? {
        get { self[RoasttySurfaceViewKey.self] }
        set { self[RoasttySurfaceViewKey.self] = newValue }
    }

    var roasttyLastFocusedSurface: Weak<Roastty.SurfaceView>? {
        get { self[RoasttyLastFocusedSurfaceKey.self] }
        set { self[RoasttyLastFocusedSurfaceKey.self] = newValue }
    }
}

extension View {
    func roasttySurfaceView(_ surfaceView: Roastty.SurfaceView?) -> some View {
        environment(\.roasttySurfaceView, surfaceView)
    }

    /// The most recently focused surface (can be currently focused if the surface is currently focused).
    func roasttyLastFocusedSurface(_ surfaceView: Weak<Roastty.SurfaceView>?) -> some View {
        environment(\.roasttyLastFocusedSurface, surfaceView)
    }
}
...
extension FocusedValues {
    var roasttySurfaceView: Roastty.SurfaceView? {
        get { self[FocusedRoasttySurface.self] }
        set { self[FocusedRoasttySurface.self] = newValue }
    }

    struct FocusedRoasttySurface: FocusedValueKey {
        typealias Value = Roastty.SurfaceView
    }

    var roasttySurfacePwd: String? {
        get { self[FocusedRoasttySurfacePwd.self] }
        set { self[FocusedRoasttySurfacePwd.self] = newValue }
    }

    struct FocusedRoasttySurfacePwd: FocusedValueKey {
        typealias Value = String
    }

    var roasttySurfaceCellSize: OSSize? {
        get { self[FocusedRoasttySurfaceCellSize.self] }
        set { self[FocusedRoasttySurfaceCellSize.self] = newValue }
    }

    struct FocusedRoasttySurfaceCellSize: FocusedValueKey {
