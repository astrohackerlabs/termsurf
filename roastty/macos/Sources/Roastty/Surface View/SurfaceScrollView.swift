...
/// Wraps a Roastty surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual Roastty renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    private let scrollView: NSScrollView
    private let documentView: NSView
    private let surfaceView: Roastty.SurfaceView
    ...
    init(contentSize: CGSize, surfaceView: Roastty.SurfaceView) {
        ...
        // so that our primary Roastty renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from Roastty
        observers.append(NotificationCenter.default.addObserver(
            forName: .roasttyDidUpdateScrollbar,
            object: surfaceView,
            queue: .main
        ) { [weak self] notification in
    ...
    private func handleScrollbarUpdate(_ notification: Notification) {
        guard let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey] as? Roastty.Action.Scrollbar else {
            return
        }
