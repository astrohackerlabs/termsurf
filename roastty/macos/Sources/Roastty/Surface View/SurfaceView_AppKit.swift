...
extension Roastty {
    ...
    class SurfaceView: OSSurfaceView, Codable, Identifiable {
                ...
                if let searchState {
                    ...
                    searchNeedleCancellable = searchState.$needle
                        ...
                        .sink { [weak self] needle in
                            ...
                            roastty_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8)))
                        }
        ...
        // detected with the set_password_input_cb on the Roastty state.
        ...
        var needsConfirmQuit: Bool {
            ...
            return roastty_surface_needs_confirm_quit(surface)
        }

        // Returns true if the process in this surface has exited.
        var processExited: Bool {
            guard let surface = self.surface else { return true }
            return roastty_surface_process_exited(surface)
        }

        // Returns the inspector instance for this surface, or nil if the
        // surface has been closed or no inspector is active.
        var inspector: Roastty.Inspector? {
            guard let surface = self.surface else { return nil }
            guard let cInspector = roastty_surface_inspector(surface) else { return nil }
            return Roastty.Inspector(cInspector: cInspector)
        }
        ...
        private(set) var surfaceModel: Roastty.Surface?

        /// Returns the underlying C value for the surface. See "note" on surfaceModel.
        override var surface: roastty_surface_t? {
        ...
        var scrollbar: Roastty.Action.Scrollbar?
        ...
        init(_ app: roastty_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            self.markedText = NSMutableAttributedString()

            // Our initial config always is our application wide config.
            if let appDelegate = NSApplication.shared.delegate as? AppDelegate {
                self.derivedConfig = DerivedConfig(appDelegate.roastty.config)
            } else {
            ...
            cachedScreenContents = .init(duration: .milliseconds(500)) { [weak self] in
                ...
                let sel = roastty_selection_s(
                    top_left: roastty_point_s(
                        tag: ROASTTY_POINT_SCREEN,
                        coord: ROASTTY_POINT_COORD_TOP_LEFT,
                        x: 0,
                        y: 0),
                    bottom_right: roastty_point_s(
                        tag: ROASTTY_POINT_SCREEN,
                        coord: ROASTTY_POINT_COORD_BOTTOM_RIGHT,
                        x: 0,
                        y: 0),
                    rectangle: false)
                guard roastty_surface_read_text(surface, sel, &text) else { return "" }
            ...
            center.addObserver(
                ...
                name: Roastty.Notification.didUpdateRendererHealth,
                object: self)
            center.addObserver(
                self,
                selector: #selector(roasttyDidContinueKeySequence),
                name: Roastty.Notification.didContinueKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(roasttyDidEndKeySequence),
                name: Roastty.Notification.didEndKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(roasttyDidChangeKeyTable),
                name: Roastty.Notification.didChangeKeyTable,
                object: self)
            center.addObserver(
                self,
                selector: #selector(roasttyConfigDidChange(_:)),
                name: .roasttyConfigDidChange,
                object: self)
            center.addObserver(
                self,
                selector: #selector(roasttyColorDidChange(_:)),
                name: .roasttyColorDidChange,
                object: self)
            center.addObserver(
                self,
                selector: #selector(roasttyBellDidRing(_:)),
            ...
            let surface = surface_cfg.withCValue(view: self) { surface_cfg_c in
                roastty_surface_new(app, &surface_cfg_c)
            }
            guard let surface = surface else {
                self.error = Roastty.Error.apiFailed
                return
            }
            self.surfaceModel = Roastty.Surface(cSurface: surface)
        ...
        override func focusDidChange(_ focused: Bool) {
            ...
            // Notify libroastty
            roastty_surface_set_focus(surface, focused)
        ...
        override func sizeDidChange(_ size: CGSize) {
            // Roastty wants to know the actual framebuffer size... It is very important
        ...
        private func setSurfaceSize(width: UInt32, height: UInt32) {
            ...
            roastty_surface_set_size(surface, width, height)
        ...
        func setCursorShape(_ shape: roastty_action_mouse_shape_e) {
            switch shape {
            case ROASTTY_MOUSE_SHAPE_DEFAULT:
                pointerStyle = .default

            case ROASTTY_MOUSE_SHAPE_TEXT:
                pointerStyle = .horizontalText

            case ROASTTY_MOUSE_SHAPE_GRAB:
                pointerStyle = .grabIdle

            case ROASTTY_MOUSE_SHAPE_GRABBING:
                pointerStyle = .grabActive

            case ROASTTY_MOUSE_SHAPE_POINTER:
                pointerStyle = .link

            case ROASTTY_MOUSE_SHAPE_W_RESIZE:
                pointerStyle = .resizeLeft

            case ROASTTY_MOUSE_SHAPE_E_RESIZE:
                pointerStyle = .resizeRight

            case ROASTTY_MOUSE_SHAPE_N_RESIZE:
                pointerStyle = .resizeUp

            case ROASTTY_MOUSE_SHAPE_S_RESIZE:
                pointerStyle = .resizeDown

            case ROASTTY_MOUSE_SHAPE_NS_RESIZE:
                pointerStyle = .resizeUpDown

            case ROASTTY_MOUSE_SHAPE_EW_RESIZE:
                pointerStyle = .resizeLeftRight

            case ROASTTY_MOUSE_SHAPE_VERTICAL_TEXT:
                pointerStyle = .verticalText

            case ROASTTY_MOUSE_SHAPE_CONTEXT_MENU:
                pointerStyle = .contextMenu

            case ROASTTY_MOUSE_SHAPE_CROSSHAIR:
                pointerStyle = .crosshair

            case ROASTTY_MOUSE_SHAPE_NOT_ALLOWED:
        ...
        @objc private func onUpdateRendererHealth(notification: SwiftUI.Notification) {
            ...
            guard let health = healthAny as? roastty_action_renderer_health_e else { return }
            DispatchQueue.main.async { [weak self] in
                self?.healthy = health == ROASTTY_RENDERER_HEALTH_HEALTHY
            }
        }

        @objc private func roasttyDidContinueKeySequence(notification: SwiftUI.Notification) {
            guard let keyAny = notification.userInfo?[Roastty.Notification.KeySequenceKey] else { return }
        ...
        @objc private func roasttyDidEndKeySequence(notification: SwiftUI.Notification) {
            DispatchQueue.main.async { [weak self] in
                self?.keySequence = []
            }
        }

        @objc private func roasttyDidChangeKeyTable(notification: SwiftUI.Notification) {
            guard let action = notification.userInfo?[Roastty.Notification.KeyTableKey] as? Roastty.Action.KeyTable else { return }
        ...
        @objc private func roasttyConfigDidChange(_ notification: SwiftUI.Notification) {
            // Get our managed configuration object out
            guard let config = notification.userInfo?[
                SwiftUI.Notification.Name.RoasttyConfigChangeKey
        ...
        @objc private func roasttyColorDidChange(_ notification: SwiftUI.Notification) {
            guard let change = notification.userInfo?[
                SwiftUI.Notification.Name.RoasttyColorChangeKey
            ] as? Roastty.Action.ColorChange else { return }
        ...
        @objc private func windowDidChangeScreen(notification: SwiftUI.Notification) {
            ...
            // When the window changes screens, we need to update libroastty with the screen
            // ID. If vsync is enabled, this will be used with the CVDisplayLink to ensure
            // the proper refresh rate is going.
            roastty_surface_set_display_id(surface, screen.displayID ?? 0)
            ...
            // Issue: https://github.com/roastty-org/roastty/issues/2731
            DispatchQueue.main.async { [weak self] in
        ...
        override func viewDidChangeBackingProperties() {
            ...
            roastty_surface_set_content_scale(surface, xScale, yScale)
        ...
        override func mouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Roastty.roasttyMods(event.modifierFlags)
            roastty_surface_mouse_button(surface, ROASTTY_MOUSE_PRESS, ROASTTY_MOUSE_LEFT, mods)
        }
        ...
        override func mouseUp(with event: NSEvent) {
            ...
            guard let surface = self.surface else { return }
            let mods = Roastty.roasttyMods(event.modifierFlags)
            roastty_surface_mouse_button(surface, ROASTTY_MOUSE_RELEASE, ROASTTY_MOUSE_LEFT, mods)

            // Release pressure
            roastty_surface_mouse_pressure(surface, 0, 0)
        }

        override func otherMouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Roastty.roasttyMods(event.modifierFlags)
            let button = Roastty.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            roastty_surface_mouse_button(surface, ROASTTY_MOUSE_PRESS, button.cMouseButton, mods)
        }
        ...
        override func otherMouseUp(with event: NSEvent) {
            ...
            let mods = Roastty.roasttyMods(event.modifierFlags)
            let button = Roastty.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            roastty_surface_mouse_button(surface, ROASTTY_MOUSE_RELEASE, button.cMouseButton, mods)
        }
        ...
        override func rightMouseDown(with event: NSEvent) {
            ...
            let mods = Roastty.roasttyMods(event.modifierFlags)
            if roastty_surface_mouse_button(
                surface,
                ROASTTY_MOUSE_PRESS,
                ROASTTY_MOUSE_RIGHT,
                mods
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            let scrollEvent = Roastty.Input.MouseScrollEvent(
                x: x,
                y: y,
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify Roastty first. We do this because this will let Roastty handle
            // state setup that we'll need for later pressure handling (such as
            // QuickLook)
            roastty_surface_mouse_pressure(surface, UInt32(event.stage), Double(event.pressure))
            ...
            guard UserDefaults.roastty.bool(forKey: "com.apple.trackpad.forceClick") else { return }
        ...
        override func keyDown(with event: NSEvent) {
            ...
            let translationModsRoastty = Roastty.eventModifierFlags(
                mods: roastty_surface_key_translation_mods(
                    surface,
                    Roastty.roasttyMods(event.modifierFlags)
                )
            )

            // There are hidden bits set in our event that matter for certain dead keys
            // so we can't use translationModsRoastty directly. Instead, we just check
            // for exact states and set them.
            var translationMods = event.modifierFlags
            for flag in [NSEvent.ModifierFlags.shift, .control, .option, .command] {
                if translationModsRoastty.contains(flag) {
            ...
            let action = event.isARepeat ? ROASTTY_ACTION_REPEAT : ROASTTY_ACTION_PRESS
            ...
            if markedTextBefore,
               ...
               list.count > 0 {
                for text in list {
                    if Roastty.SurfaceView.shouldSuppressComposingControlInput(
                        text,
                        composing: composing
                    ) {
                        continue
            ...
            if let list = keyTextAccumulator, list.count > 0 {
                ...
                _ = keyAction(
                    ...
                    event: event,
                    translationEvent: translationEvent,
                    text: translationEvent.roasttyCharacters,
                    composing: composing
                )
            }
        }
        ...
        override func keyUp(with event: NSEvent) {
            _ = keyAction(ROASTTY_ACTION_RELEASE, event: event)
        }
        ...
        override func performKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            let bindingFlags = surfaceModel.flatMap { surface in
                var roasttyEvent = event.roasttyKeyEvent(ROASTTY_ACTION_PRESS)
                return (event.characters ?? "").withCString { ptr in
                    roasttyEvent.text = ptr
                    return surface.keyIsBinding(roasttyEvent)
                }
            }
                ...
                if keySequence.isEmpty,
                   ...
                   bindingFlags.contains(.consumed) {
                    if let appDelegate = NSApp.delegate as? AppDelegate,
                       appDelegate.performRoasttyBindingMenuKeyEquivalent(with: event) {
                        return true
                    }
                }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            case 0x39: mod = ROASTTY_MODS_CAPS.rawValue
            case 0x38, 0x3C: mod = ROASTTY_MODS_SHIFT.rawValue
            case 0x3B, 0x3E: mod = ROASTTY_MODS_CTRL.rawValue
            case 0x3A, 0x3D: mod = ROASTTY_MODS_ALT.rawValue
            case 0x37, 0x36: mod = ROASTTY_MODS_SUPER.rawValue
            default: return
            }
            ...
            var action = ROASTTY_ACTION_RELEASE
        ...
        private func keyAction(
            _ action: roastty_input_action_e,
            event: NSEvent,
            ...
            var key_ev = event.roasttyKeyEvent(action, translationMods: translationEvent?.modifierFlags)
            key_ev.composing = composing

            // For text, we only encode UTF8 if we don't have a single control
            // character. Control characters are encoded by Roastty itself.
        ...
        private func shouldReplayCommittedPreeditKey(_ event: NSEvent) -> Bool {
            guard let key = Roastty.Input.Key(keyCode: event.keyCode) else { return false }
            switch key {
        ...
        private func committedPreeditTextAction(
            ...
            var key_ev = roastty_input_key_s()
            key_ev.action = action
            key_ev.keycode = 0
            key_ev.text = nil
            key_ev.composing = false
            key_ev.mods = ROASTTY_MODS_NONE
            key_ev.consumed_mods = ROASTTY_MODS_NONE
        ...
        @IBAction func splitDown(_ sender: Any) {
            guard let surface = self.surface else { return }
            roastty_surface_split(surface, ROASTTY_SPLIT_DIRECTION_DOWN)
        }

        @IBAction func splitUp(_ sender: Any) {
            guard let surface = self.surface else { return }
            roastty_surface_split(surface, ROASTTY_SPLIT_DIRECTION_UP)
        }
        ...
        func showUserNotification(title: String, body: String, requireFocus: Bool = true) {
            ...
            content.categoryIdentifier = Roastty.userNotificationCategory
            content.userInfo = [
        ...
        struct DerivedConfig {
            ...
            let backgroundBlur: Roastty.Config.BackgroundBlur
            let macosWindowShadow: Bool
            let windowTitleFontFamily: String?
            let windowAppearance: NSAppearance?
            let scrollbar: Roastty.Config.Scrollbar
            ...
            init(_ config: Roastty.Config) {
                ...
                self.windowAppearance = .init(roasttyConfig: config)
...
extension Roastty.SurfaceView: NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        // Get our range from the Roastty API. There is a race condition between getting the
        ...
        var text = roastty_text_s()
        guard roastty_surface_read_selection(surface, &text) else { return NSRange() }
        defer { roastty_surface_free_text(surface, &text) }
    ...
    func attributedSubstring(forProposedRange range: NSRange, actualRange: NSRangePointer?) -> NSAttributedString? {
        // Roastty.logger.warning("pressure substring range=\(range) selectedRange=\(self.selectedRange())")
        guard let surface = self.surface else { return nil }
        ...
        var text = roastty_text_s()
        guard roastty_surface_read_selection(surface, &text) else { return nil }
        defer { roastty_surface_free_text(surface, &text) }
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // Roastty will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        if range.length > 0 && range != self.selectedRange() {
            // QuickLook
            var text = roastty_text_s()
            if roastty_surface_read_selection(surface, &text) {
                ...
                // Free our text
                roastty_surface_free_text(surface, &text)
            } else {
                roastty_surface_ime_point(surface, &x, &y, &width, &height)
            }
        } else {
            roastty_surface_ime_point(surface, &x, &y, &width, &height)
        }
        ...
        // Roastty coordinates are in top-left (0, 0) so we have to convert to
    ...
    /// Sync the preedit state based on the markedText value to libroastty
...
extension Roastty.SurfaceView: NSServicesMenuRequestor {
    override func validRequestor(
        ...
        if (returnType == nil || receivable.contains(returnType!)) &&
            ...
            // performance hits because `roastty_surface_has_selection` isn't free.
            if let sendType, sendableRequiresSelection.contains(sendType) {
                if surface == nil || !roastty_surface_has_selection(surface) {
    ...
    func writeSelection(
        ...
        // Read the selection
        var text = roastty_text_s()
        guard roastty_surface_read_selection(surface, &text) else { return false }
        defer { roastty_surface_free_text(surface, &text) }
    ...
    func readSelection(from pboard: NSPasteboard) -> Bool {
        ...
        str.withCString { ptr in
            ...
            roastty_surface_text(surface, ptr, UInt(len - 1))
        }
...
extension Roastty.SurfaceView: NSMenuItemValidation {
    func validateMenuItem(_ item: NSMenuItem) -> Bool {
        switch item.action {
        case #selector(pasteSelection):
            let pb = NSPasteboard.roasttySelection
...
extension Roastty.SurfaceView {
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        if let url = pb.string(forType: .URL) {
            ...
            content = Roastty.Shell.escape(url)
           ...
           urls.count > 0 {
            ...
            content = urls
                .map { Roastty.Shell.escape($0.path) }
                .joined(separator: " ")
...
extension Roastty.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment Roastty core to
    /// expose that.
    ///
