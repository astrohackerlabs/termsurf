...
extension Roastty {
    class OSSurfaceView: OSView, ObservableObject {
        ...
        @Published var surfaceSize: roastty_surface_size_s?
        ...
        /// A message sent from `roastty_surface_t` when a child process exited
        @Published private(set) var childExitedMessage: ChildExitedMessage?

        var surface: roastty_surface_t? {
            nil
        }
        ...
        init(id: UUID?, frame: CGRect) {
            ...
            center.addObserver(
                self,
                selector: #selector(roasttyDidChangeReadonly(_:)),
        ...
        @objc private func roasttyDidChangeReadonly(_ notification: Foundation.Notification) {
...
extension Roastty.OSSurfaceView {
    @MainActor class SearchState: ObservableObject {
        ...
        init(
            from startSearch: Roastty.Action.StartSearch,
