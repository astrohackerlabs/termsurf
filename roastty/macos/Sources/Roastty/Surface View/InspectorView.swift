...
extension Roastty {
    ...
    struct InspectableSurface: View {
        @EnvironmentObject var roastty: Roastty.App
        ...
        var body: some View {
            ...
            ZStack {
                if !surfaceView.inspectorVisible {
                    ...
                    SplitView(.vertical, $split, dividerColor: roastty.config.splitDividerColor, left: {
                        SurfaceWrapper(surfaceView: surfaceView, isSplit: isSplit)
                    }, right: {
        ...
        private func onControlInspector(_ notification: SwiftUI.Notification) {
            ...
            guard let mode = modeAny as? roastty_action_inspector_e else { return }

            switch mode {
            case ROASTTY_INSPECTOR_TOGGLE:
                surfaceView.inspectorVisible = !surfaceView.inspectorVisible

            case ROASTTY_INSPECTOR_SHOW:
                surfaceView.inspectorVisible = true

            case ROASTTY_INSPECTOR_HIDE:
    ...
    class InspectorView: MTKView, NSTextInputClient {
        ...
        private var inspector: Roastty.Inspector? {
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            inspector.mouseButton(ROASTTY_MOUSE_PRESS, button: ROASTTY_MOUSE_LEFT, mods: mods)
        }

        override func mouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = Roastty.roasttyMods(event.modifierFlags)
            inspector.mouseButton(ROASTTY_MOUSE_RELEASE, button: ROASTTY_MOUSE_LEFT, mods: mods)
        }

        override func rightMouseDown(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = Roastty.roasttyMods(event.modifierFlags)
            inspector.mouseButton(ROASTTY_MOUSE_PRESS, button: ROASTTY_MOUSE_RIGHT, mods: mods)
        }

        override func rightMouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = Roastty.roasttyMods(event.modifierFlags)
            inspector.mouseButton(ROASTTY_MOUSE_RELEASE, button: ROASTTY_MOUSE_RIGHT, mods: mods)
        }
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            var momentum: roastty_input_mouse_momentum_e = ROASTTY_MOUSE_MOMENTUM_NONE
            switch event.momentumPhase {
            case .began:
                momentum = ROASTTY_MOUSE_MOMENTUM_BEGAN
            case .stationary:
                momentum = ROASTTY_MOUSE_MOMENTUM_STATIONARY
            case .changed:
                momentum = ROASTTY_MOUSE_MOMENTUM_CHANGED
            case .ended:
                momentum = ROASTTY_MOUSE_MOMENTUM_ENDED
            case .cancelled:
                momentum = ROASTTY_MOUSE_MOMENTUM_CANCELLED
            case .mayBegin:
                momentum = ROASTTY_MOUSE_MOMENTUM_MAY_BEGIN
            default:
                break
            }
        ...
        override func keyUp(with event: NSEvent) {
            keyAction(ROASTTY_ACTION_RELEASE, event: event)
        }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            if mods.rawValue & mod != 0 { action = ROASTTY_ACTION_PRESS }

            keyAction(action, event: event)
        }

        private func keyAction(_ action: roastty_input_action_e, event: NSEvent) {
            guard let inspector = self.inspector else { return }
            guard let key = Roastty.Input.Key(keyCode: event.keyCode) else { return }
            let mods = Roastty.roasttyMods(event.modifierFlags)
