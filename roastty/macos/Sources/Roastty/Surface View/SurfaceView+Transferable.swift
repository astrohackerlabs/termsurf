...
extension Roastty.SurfaceView: Transferable {
    static var transferRepresentation: some TransferRepresentation {
        DataRepresentation(contentType: .roasttySurfaceId) { surface in
    ...
    static func find(uuid: UUID) -> Self? {
        #if canImport(AppKit)
        guard let del = NSApp.delegate as? Roastty.Delegate else { return nil }
        return del.roasttySurface(id: uuid) as? Self
...
extension UTType {
    ...
    static let roasttySurfaceId = UTType(exportedAs: "com.mitchellh.roasttySurfaceId")
}

#if canImport(AppKit)
extension NSPasteboard.PasteboardType {
    /// Pasteboard type for dragging surface IDs.
    static let roasttySurfaceId = NSPasteboard.PasteboardType(UTType.roasttySurfaceId.identifier)
}
#endif
