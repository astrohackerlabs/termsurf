...
extension Roastty {
    ...
    final class Surface: Sendable {
        ...
        var unsafeCValue: roastty_surface_t {
            surface
        }

        /// Initialize from the C structure.
        init(cSurface: roastty_surface_t) {
            self.surface = cSurface
        }

        deinit {
            // deinit is not guaranteed to happen on the main actor and our API
            // calls into libroastty must happen there so we capture the surface
        ...
        func sendKeyEvent(_ event: Input.KeyEvent) {
            event.withCValue { cEvent in
                roastty_surface_key(surface, cEvent)
            }
        }
        ...
        @MainActor
        func keyIsBinding(_ event: roastty_input_key_s) -> Input.BindingFlags? {
            var flags = roastty_binding_flags_e(0)
            guard roastty_surface_key_is_binding(surface, event, &flags) else { return nil }
            return Input.BindingFlags(cFlags: flags)
        }

        /// See `keyIsBinding(_ event: roastty_input_key_s)`.
        @MainActor
        ...
        var mouseCaptured: Bool {
            roastty_surface_mouse_captured(surface)
        }

        /// The PID of the foreground process group attached to the PTY.
        @MainActor
        var foregroundPID: Int? {
            let pid = roastty_surface_foreground_pid(surface)
        ...
        @MainActor
        var ttyName: String? {
            let ttyName = AllocatedString(roastty_surface_tty_name(surface)).string
