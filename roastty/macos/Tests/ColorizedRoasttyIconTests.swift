...
struct ColorizedRoasttyIconTests {
    private func makeIcon(
        ...
        frame: Roastty.MacOSIconFrame = .aluminum
    ) -> ColorizedRoasttyIcon {
    ...
    @Test func codableRoundTripPreservesIcon() throws {
        ...
        let decoded = try JSONDecoder().decode(ColorizedRoasttyIcon.self, from: data)
    ...
    @Test func decodingUnsupportedVersionThrowsDataCorrupted() {
        ...
        do {
            _ = try JSONDecoder().decode(ColorizedRoasttyIcon.self, from: data)
            Issue.record("Expected decode to fail for unsupported version")
        } catch let DecodingError.dataCorrupted(context) {
            #expect(context.debugDescription.contains("Unsupported ColorizedRoasttyIcon version"))
        } catch {
