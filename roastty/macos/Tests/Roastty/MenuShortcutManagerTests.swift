...
struct MenuShortcutManagerTests {
    @Test(.bug("https://github.com/roastty-org/roastty/issues/779", id: 779))
    func unbindShouldDiscardDefault() async throws {
        ...
        let manager = await Roastty.MenuShortcutManager()
        await manager.reset()
    ...
    @Test(.bug("https://github.com/roastty-org/roastty/issues/11396", id: 11396))
    func overrideDefault() async throws {
        let config = try TemporaryConfig("keybind=super+h=goto_split:left")

        let hideItem = NSMenuItem(title: "Hide Roastty", action: "hide:", keyEquivalent: "h")
