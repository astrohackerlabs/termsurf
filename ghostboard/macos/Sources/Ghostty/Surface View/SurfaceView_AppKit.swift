...
extension Ghostty {
  ...
  class SurfaceView: OSSurfaceView, Codable, Identifiable {
    ...
    @Published var termsurfCopyUrlFeedback: String? {
      didSet {
        ...
        if termsurfCopyUrlFeedback != nil {
          termsurfCopyUrlFeedbackTimer = Timer.scheduledTimer(withTimeInterval: 1.5, repeats: false)
          { [weak self] _ in
    ...
    private struct TermSurfIOSurfaceOverlayRequest {
    ...
    private var termsurfOverlayIOSurfaceLayer: CALayer?
    private var termsurfOverlayIOSurface: IOSurfaceRef?
    ...
    func termSurfGeometryIdentity(browserTabID: String = "unknown:appkit") -> String {
      ...
      let selectedTabID =
        window?.tabGroup?.selectedWindow.map { String($0.windowNumber) } ?? windowID
    ...
    private func termSurfLogGeometry(
      ...
      let visibleValue = visible ?? (termsurfOverlayHostLayer != nil)
    ...
    private func applyTermSurfOverlay(
      ...
      let positioning =
        termsurfOverlayPositioningLayer
        ?? {
          let layer = CALayer()
    ...
    func presentTermSurfIOSurfaceOverlay(
      ...
      let grid = "\(width)x\(height)+\(col)+\(row)"
      let browserPixel = "\(pixelWidth)x\(pixelHeight)"
      let request = TermSurfIOSurfaceOverlayRequest(
        surface: surface,
        col: col,
        row: row,
        width: width,
      ...
      guard isTermSurfSelectedTab else {
        ...
        removeTermSurfOverlayLayers(resetDeferred: false)
        termSurfLogGeometry(
          event: "present_iosurface_rejected",
          grid: grid,
          browserPixel: browserPixel,
      ...
      guard let surfaceLayer = layer else {
        ...
        AppDelegate.logger.warning(
          "TermSurf IOSurface overlay rejected: surface has no backing layer")
        termSurfLogGeometry(
          event: "present_iosurface_rejected",
          grid: grid,
          browserPixel: browserPixel,
      ...
      termsurfDeferredOverlay = nil
      termsurfActiveOverlay = nil
      if !applyTermSurfIOSurfaceOverlay(request, surfaceLayer: surfaceLayer) {
        Unmanaged.passUnretained(surface).release()
      }
    }
    ...
    private func applyTermSurfIOSurfaceOverlay(
      ...
      let frame = CGRect(
        ...
        height: CGFloat(request.height) * cellHeight)
      guard frame.width > 0, frame.height > 0 else {
        AppDelegate.logger.warning("TermSurf IOSurface overlay rejected: empty frame")
        termSurfLogGeometry(
          event: "present_iosurface_rejected",
          grid: grid,
          browserPixel: browserPixel,
          contextID: request.attachmentID,
          note: "empty-frame")
        return false
      }
      ...
      let iosurfaceLayer =
        termsurfOverlayIOSurfaceLayer
        ?? {
          let layer = CALayer()
          layer.name = "TermSurfOverlayIOSurface"
          layer.anchorPoint = .zero
          positioning.addSublayer(layer)
          termsurfOverlayIOSurfaceLayer = layer
          return layer
        }()
      ...
      let appkitPixelHeight = UInt64((frame.height * backingScale).rounded())
      termSurfLogGeometry(
        event: "presented_iosurface",
        grid: grid,
        browserPixel: browserPixel,
        contextID: request.attachmentID,
        visible: termsurfOverlayIOSurfaceLayer != nil,
        note: "iosurface-overlay-presented")
      termsurfLogGeometry(
        "layer=appkit event=presented_iosurface_pixels scenario=\(termsurfGeometryScenario()) identity=\(termSurfGeometryIdentity()) grid=\(grid) browser_pixel=\(browserPixel) appkit_pixel=\(appkitPixelWidth)x\(appkitPixelHeight) backing_scale=\(backingScale) attachment_id=\(request.attachmentID) generation=\(request.generation) visible=true note=reported-presented-iosurface-pixels"
      )
      id.uuidString.withCString { paneIDPointer in
        termsurf_overlay_presented_pixels(
          paneIDPointer,
      ...
      AppDelegate.logger.info(
        "TermSurf IOSurface overlay presented pane_id=\(self.id.uuidString) attachment_id=\(request.attachmentID) generation=\(request.generation) frame=\(NSStringFromRect(frame)) pixel=\(request.pixelWidth)x\(request.pixelHeight)"
      )
      fputs(
        "TermSurf IOSurface overlay presented pane_id=\(self.id.uuidString) attachment_id=\(request.attachmentID) generation=\(request.generation) frame=\(NSStringFromRect(frame)) pixel=\(request.pixelWidth)x\(request.pixelHeight)\n",
        stderr)
    ...
    private func releaseTermSurfIOSurfaceOverlayRetain() {
      if let surface = termsurfOverlayIOSurface {
        Unmanaged.passUnretained(surface).release()
    ...
    private func localEventLeftMouseDown(_ event: NSEvent) -> NSEvent? {
      let isCommandPaletteVisible =
        (event.window?.windowController as? BaseTerminalController)?
    ...
    @objc private func ghosttyDidChangeKeyTable(notification: SwiftUI.Notification) {
      guard
        let action = notification.userInfo?[Ghostty.Notification.KeyTableKey]
          as? Ghostty.Action.KeyTable
      else { return }
    ...
    @objc private func ghosttyColorDidChange(_ notification: SwiftUI.Notification) {
      guard
        let change = notification.userInfo?[
          SwiftUI.Notification.Name.GhosttyColorChangeKey
        ] as? Ghostty.Action.ColorChange
      else { return }
    ...
    override func mouseMoved(with event: NSEvent) {
      ...
      if let window,
        ...
        window.isKeyWindow && !self.focused && controller.focusFollowsMouse
      {
        Ghostty.moveFocus(to: self)
      }
    }
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
      ...
      if let bindingFlags {
        termSurfLogGeometry(
          ...
          note:
            "key_code=\(event.keyCode) modifiers=\(termSurfModifiers(event.modifierFlags)) focused=\(focused) flags=\(bindingFlags.rawValue)"
        )
    ...
    func termSurfScriptForwardMouseEvent(
      ...
      termSurfLogGeometry(
        ...
        note:
          "kind=mouse type=\(type) button=\(button) click_count=\(clickCount) modifiers=\(modifiers)"
      )
      return true
    }
    ...
    func termSurfScriptForwardKey(
      ...
      termSurfLogGeometry(
        event: "script_input_forwarded",
        note:
          "kind=key type=\(type) windows_key_code=\(windowsKeyCode) utf8_len=\(text.utf8.count) modifiers=\(modifiers)"
      )
      return true
    }
    ...
    private func termSurfOverlayPoint(for event: NSEvent) -> TermSurfOverlayPoint? {
      ...
      let webPoint = CGPoint(
        x: topOriginPoint.x - overlayFrame.minX, y: topOriginPoint.y - overlayFrame.minY)
      termSurfLogGeometry(
        event: "hit_test",
        hit: hit,
        rawPoint: viewPoint,
    ...
    private func forwardTermSurfMouseEvent(
      ...
      if forwarded != 0 {
        termSurfLogGeometry(
          ...
          note:
            "kind=event ns_event=\(event.type.rawValue) type=\(type) button=\(button) click_count=\(max(event.clickCount, 1)) modifiers=\(modifiers) terminal_fallback=false"
        )
      }
      return forwarded != 0
    }
    ...
    private func forwardTermSurfMouseMove(_ event: NSEvent) -> Bool {
      ...
      if forwarded != 0 {
        ...
        termSurfLogGeometry(
          ...
          webPoint: CGPoint(x: point.x, y: point.y),
          note:
            "kind=move ns_event=\(event.type.rawValue) modifiers=\(modifiers) terminal_fallback=false"
        )
      }
      return forwarded != 0
    }
    ...
    override func menu(for event: NSEvent) -> NSMenu? {
      ...
      item = menu.addItem(
        withTitle: "Split Right", action: #selector(splitRight(_:)), keyEquivalent: "")
      item.setImageIfDesired(systemSymbolName: "rectangle.righthalf.inset.filled")
      item = menu.addItem(
        withTitle: "Split Left", action: #selector(splitLeft(_:)), keyEquivalent: "")
      item.setImageIfDesired(systemSymbolName: "rectangle.leadinghalf.inset.filled")
      item = menu.addItem(
        withTitle: "Split Down", action: #selector(splitDown(_:)), keyEquivalent: "")
      ...
      menu.addItem(.separator())
      item = menu.addItem(
        withTitle: "Reset Terminal", action: #selector(resetTerminal(_:)), keyEquivalent: "")
      item.setImageIfDesired(systemSymbolName: "arrow.trianglehead.2.clockwise")
      item = menu.addItem(
        withTitle: "Toggle Terminal Inspector", action: #selector(toggleTerminalInspector(_:)),
        keyEquivalent: "")
      item.setImageIfDesired(systemSymbolName: "scope")
      item = menu.addItem(
        withTitle: "Terminal Read-only", action: #selector(toggleReadonly(_:)), keyEquivalent: "")
      item.setImageIfDesired(systemSymbolName: "eye.fill")
      item.state = readonly ? .on : .off
      menu.addItem(.separator())
      item = menu.addItem(
        withTitle: "Change Tab Title...",
        action: #selector(BaseTerminalController.changeTabTitle(_:)), keyEquivalent: "")
      item.setImageIfDesired(systemSymbolName: "pencil.line")
      item = menu.addItem(
        withTitle: "Change Terminal Title...", action: #selector(changeTitle(_:)), keyEquivalent: ""
      )
    ...
    required convenience init(from decoder: Decoder) throws {
      ...
      let isUserSetTitle =
        try container.decodeIfPresent(Bool.self, forKey: .isUserSetTitle) ?? false
