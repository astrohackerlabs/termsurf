...
//
// `ghostboard +new-window` is equivalent to the following command (on a release build):
//
// ```
...
// ```
//
// `ghostboard +new-window -e echo hello` would be equivalent to the following command (on a release build):
//
// ```
