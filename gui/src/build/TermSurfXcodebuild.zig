...
const Docs = @import("TermSurfDocs.zig");
const I18n = @import("TermSurfI18n.zig");
const Resources = @import("TermSurfResources.zig");
const XCFramework = @import("TermSurfXCFramework.zig");
...
pub fn init(
    ...
    const open = open: {
        ...
        open.setEnvironmentVariable("TERMSURF_LOG", "stderr,macos");

        // Configure how we're launching
        open.setEnvironmentVariable("TERMSURF_MAC_LAUNCH_SOURCE", "zig_run");
...
pub fn install(self: *const TermSurf) void {
    const b = self.copy.step.owner;
    b.getInstallStep().dependOn(&self.copy.step);
}

pub fn installXcframework(self: *const TermSurf) void {
