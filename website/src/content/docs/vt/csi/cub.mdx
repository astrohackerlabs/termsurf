...
cursor column and the left margin. If the cursor begins to the left of the left margin, modify the left margin to be the leftmost column
...
- **Extended reverse wrap**: wraparound (mode 7) and extended reverse wrap (mode 1045)
  are **BOTH** enabled
- **Reverse wrap**: wraparound (mode 7) and reverse wrap (mode 45)
  are **BOTH** enabled
...
leftmost boundary, if `n > 0` then move the cursor to the right margin
of the line above the cursor. If the cursor is already on the
top margin, move the cursor to the right margin of the
bottom margin. Both the cursor column and row can change in this
...
cursor to the right margin of the line above the cursor. If the previous
line was not wrapped, the cursor left operation is complete even if there
is a remaining value of `n`. If the cursor
is already on the top margin, do not move the cursor up.
