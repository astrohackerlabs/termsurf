...
navLabel: DL
description: Deletes `n` lines at the current cursor position and shifts existing lines up.
...
region if it is above the top margin, below the bottom margin,
left of the left margin, or right of the right margin.
...
If a left margin or right margin is set, only the cells
