...
This sequence requires enable left and right margin (mode 69)
to be set. If mode 69 is not set, this sequence does nothing and left
and right margins will not be set.

This sequence conflicts with save cursor (`CSI s`). If
...
the top-left of the screen. If origin mode is set, the cursor is
