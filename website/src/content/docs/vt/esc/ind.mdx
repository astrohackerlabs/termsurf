...
title: Index (IND)
navLabel: IND
description: Move the cursor down one cell, scrolling if necessary.
...
left and right margin, scroll up one line.
If the scroll region is the full terminal screen and the terminal is on
the primary screen, this may create scrollback. See the
scroll documentation for more details.

If the cursor is outside of the scroll region or not on the bottom
margin of the scroll region, perform the [cursor down](/docs/vt/csi/cud) operation with
`n = 1`.
