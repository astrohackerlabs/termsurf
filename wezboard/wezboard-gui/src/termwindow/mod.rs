...
use ::wezboard_term::input::{ClickPosition, MouseButton as TMB};
use ::window::*;
...
use wezboard_dynamic::Value;
use wezboard_font::FontConfiguration;
use wezboard_term::color::ColorPalette;
use wezboard_term::input::LastMouseClick;
use wezboard_term::{Alert, Progress, StableRowIndex, TerminalConfiguration, TerminalSize};
...
pub enum TermWindowNotif {
    ...
    GetConfigOverrides(Sender<wezboard_dynamic::Value>),
    SetConfigOverrides(wezboard_dynamic::Value),
...
pub struct TermWindow {
    ...
    pub config_overrides: wezboard_dynamic::Value,
...
impl TermWindow {
    ...
    fn created(&mut self, ctx: RenderContext) -> anyhow::Result<()> {
        ...
        match RenderState::new(ctx, &self.fonts, &self.render_metrics, ATLAS_SIZE) {
            Ok(render_state) => {
                log::debug!(
                    "OpenGL initialized! {} wezboard version: {}",
                    render_info,
...
impl TermWindow {
    pub async fn new_window(mux_window_id: MuxWindowId) -> anyhow::Result<()> {
        ...
        let myself = Self {
            ...
            window_background,
            config: config.clone(),
            config_overrides: wezboard_dynamic::Value::default(),
            palette: None,
            focused: None,
            mux_window_id,
    ...
    fn dispatch_window_event(
        ...
        match event {
            WindowEvent::Destroyed => {
                ...
                // <https://github.com/termsurf/termsurf/issues/3522>
            ...
            WindowEvent::AppearanceChanged(appearance) => {
                ...
                // <https://github.com/termsurf/termsurf/issues/2295>
                config::reload();
        ...
        match notif {
            ...
            TermWindowNotif::PerformAssignment {
                ...
                let result = || -> anyhow::Result<()> {
                    ...
                    // <https://github.com/termsurf/termsurf/issues/3209>
...
impl TermWindow {
    ...
    fn activate_tab(&mut self, tab_idx: isize) -> anyhow::Result<()> {
        ...
        // logic in wezboard/src/main.rs. If you update this, update that!
        let max = window.len();
    ...
    fn get_semantic_prompt_zones(&mut self, pane: &Arc<dyn Pane>) -> &[StableRowIndex] {
        ...
        if cache.seqno != seqno {
            ...
            let mut zones: Vec<StableRowIndex> = zones
                .into_iter()
                .filter_map(|zone| {
                    if zone.semantic_type == wezboard_term::SemanticType::Prompt {
                        Some(zone.start_y)
                    } else {
                        None
                    }
            ...
            // <https://github.com/termsurf/termsurf/issues/1121>
            zones.dedup();
            cache.zones = zones;
            cache.seqno = seqno;
        }
    ...
    pub fn perform_key_assignment(
        ...
        match assignment {
            ...
            OpenUri(link) => {
                wezboard_open_url::open_url(link);
            }
