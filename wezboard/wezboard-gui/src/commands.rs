...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    wezboard_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> =
            vec!["TermSurf Wezboard", "Shell", "Edit", "View", "Window"];
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                        ...
                        let about_item = MenuItem::new_with(
                            &format!("Wezboard {}", config::wezboard_version()),
                            Some(wezboard_perform_key_assignment_sel),
                            "",
                        );
                        ...
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::CopyTextTo {
                                text: config::wezboard_version().to_string(),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://wezboard.org/" => CommandDef {
                brief: "Documentation".into(),
                doc: "Visit the wezboard documentation website".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("md_help"),
            },
            "https://github.com/termsurf/termsurf/discussions/" => CommandDef {
                brief: "Discuss on GitHub".into(),
                doc: "Visit wezboard's GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("oct_comment_discussion"),
            },
            "https://github.com/termsurf/termsurf/issues/" => CommandDef {
                brief: "Search or report issue on GitHub".into(),
                doc: "Visit wezboard's GitHub issues".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("fa_ticket"),
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    return vec![
        ...
        ShowTabNavigator,
        // ----------------- Help
        OpenUri("https://wezboard.org/".to_string()),
        OpenUri("https://github.com/termsurf/termsurf/discussions/".to_string()),
        OpenUri("https://github.com/termsurf/termsurf/issues/".to_string()),
        ShowDebugOverlay,
        // ----------------- Misc
