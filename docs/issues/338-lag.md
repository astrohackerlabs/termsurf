...
**Status:** NEW — High priority based on Experiment 1 findings.
...
Instrument the full pipeline to measure where time is spent:

1. **CEF frame production** — In `termsurf-profile/src/main.rs`, in
   `on_accelerated_paint`:
   ```rust
   static LAST_FRAME: OnceLock<Mutex<Instant>> = OnceLock::new();
   let last = LAST_FRAME.get_or_init(|| Mutex::new(Instant::now()));
   let mut last_guard = last.lock().unwrap();
   let interval = last_guard.elapsed();
   *last_guard = Instant::now();
   log::info!("[PERF-CEF] Frame produced, interval={:?}", interval);
   ```

2. **XPC send timestamp** — Log when frame is sent via XPC:
   ```rust
   log::info!("[PERF-CEF] XPC frame sent at {:?}", Instant::now());
   ```

3. **XPC receive timestamp** — In GUI's XPC handler:
   ```rust
   log::info!("[PERF-GUI] XPC frame received at {:?}", Instant::now());
   ```

4. **GUI redraw trigger** — Log what triggers each redraw:
   ```rust
   log::info!("[PERF-GUI] Redraw triggered by: {:?}", trigger_source);
   ```

**Questions to answer:**

- [ ] Is CEF producing frames at 60fps or slower?
- [ ] Is XPC transport adding latency between send and receive?
- [ ] Is the GUI redrawing on every new frame, or batching/dropping frames?
- [ ] Are there specific operations causing the large gaps (149ms, 404ms, 459ms)?

#### Phase 2: Analyze Data

Compare timestamps across the pipeline:

| Stage | Expected Interval | Measured |
|-------|-------------------|----------|
| CEF frame production | 16ms | ? |
| XPC transport latency | <5ms | ? |
| GUI frame receive to render | <1ms | ? |

#### Phase 3: Fix Based on Findings

Depends on what Phase 1 reveals:

- If CEF is slow → Investigate CEF message loop, `windowless_frame_rate` setting
- If XPC is slow → Batch frames, use different XPC primitives, or shared memory
- If GUI is slow → Fix redraw triggers, ensure immediate redraw on new frame

**Success criteria:**

- [ ] Frame intervals reduced from 50ms to ~16ms
- [ ] Large gaps (>50ms) eliminated
- [ ] Scrolling feels smooth at 60fps

---

### Experiment 3: Throttle Mouse Move Events
...
1. **Count mouse move events** — In `mouseevent.rs`:
   ```rust
   static MOUSE_MOVE_COUNT: AtomicU64 = AtomicU64::new(0);
   // In handler:
   let count = MOUSE_MOVE_COUNT.fetch_add(1, Ordering::Relaxed);
   if count % 100 == 0 {
       log::info!("[PERF] Mouse moves sent: {}", count);
   }
   ```

2. **Log in profile server** — Count mouse tasks posted to CEF UI thread

**Baseline data to collect:**

- [ ] Mouse move events per second during active movement
- [ ] CEF UI thread task queue depth (if measurable)

#### Phase 2: Implement Throttling

1. Add `last_mouse_move: HashMap<PaneId, Instant>` to track timing
2. Skip `send_mouse_move()` if <16ms since last send for same pane
3. On skip, store pending position; send on next allowed interval
4. Always send final position when mouse stops (use 20ms timer)

#### Phase 3: Measure Improvement

- [ ] Mouse move events reduced from 100+/sec to ~60/sec
- [ ] Hover effects still feel responsive (no visible delay)
- [ ] No "lost" mouse positions causing visual glitches
- [ ] Frame rate during mouse movement improved

---

### Experiment 4: Reduce Frame Latency
...
1. **Frame presentation timing** — In `do_paint_webgpu()`:
   ```rust
   let present_start = std::time::Instant::now();
   output.present();
   log::info!("[PERF] Frame present: {:?}", present_start.elapsed());
   ```

2. **Input-to-frame correlation** — Tag mouse events with timestamp, log when
   corresponding frame is presented (requires passing timestamp through XPC)

**Baseline data to collect:**

- [ ] Frame presentation time (how long `present()` blocks)
- [ ] Input-to-display latency estimate

#### Phase 2: Change Configuration

1. Change `desired_maximum_frame_latency: 1` in webgpu.rs:377
2. Test with current `PresentMode::Fifo`
3. If no improvement, try `PresentMode::Mailbox`

#### Phase 3: Measure Improvement

- [ ] Frame presentation time comparison
- [ ] Subjective input responsiveness test
- [ ] Check for frame drops or tearing
- [ ] Scrolling latency comparison
