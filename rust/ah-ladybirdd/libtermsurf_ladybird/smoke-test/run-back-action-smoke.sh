...
RUN_DIR="$(mktemp -d "${TMPDIR:-/tmp}/ladybird-back-action.XXXXXX")"
SERVER_PID=""
...
case "$(git -C "$FORK" rev-parse --abbrev-ref HEAD)" in
  ...
  *)
    printf '%s\n' "Ladybird navigation smoke requires a compatible issue branch" >&2
    exit 1
    ;;
esac
...
(
  ...
  TERMSURF_LADYBIRD_BACKEND=real \
    ...
    cargo run -p ah-ladybirdd -- --termsurf-back-action-smoke
)
