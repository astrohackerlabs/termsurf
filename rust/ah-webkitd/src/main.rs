...
unsafe extern "C" fn on_initialized(_user_data: *mut c_void) {
    ...
    let Some(path) = SOCKET_PATH.get() else {
        eprintln!("[WebKit] No --ipc-socket, skipping IPC");
...
fn startup_trace(event: &str) {
    ...
    let browser = BROWSER_NAME.get().map(String::as_str).unwrap_or("webkit");
    let listen_socket = LISTEN_PATH.get().map(String::as_str).unwrap_or("");
    let ipc_socket = SOCKET_PATH.get().map(String::as_str).unwrap_or("");
    let line = format!(
        "TermSurfEngineStartup event={event} engine=webkit browser={browser} profile={profile} pid={pid} wall_ms={} elapsed_ms={elapsed_ms} listen_socket={listen_socket} ipc_socket={ipc_socket}",
        startup_wall_ms()
    );
...
fn main() {
    ...
    for arg in process_args.iter().skip(1) {
        if let Some(val) = arg.strip_prefix("--ipc-socket=") {
            ...
            eprintln!("[WebKit] accepted optional host render service: {val}");
    ...
    // The render service belongs to the host/wrapper contract. WebKit keeps
    // using its CAContext transport, so do not leak the host-only option into
    // WebKit's command-line parser.
    ...
    unsafe {
        ...
        if std::env::var_os("ASTROHACKER_WEBKIT_RENDER_PROOF_TRACE_FILE").is_some() {
    ...
    S: AsRef<str>,
{
    for arg in args {
        match arg.as_ref() {
            ...
            "--help" | "-h" => {
                print!(
                    "Astrohacker WebKit Engine — WebKit support helper for Astrohacker TermSurf\n\n\
...
mod tests {
    ...
    #[test]
    fn host_render_service_does_not_reach_webkit() {
        let args = upstream_engine_args([
            "/tmp/build/arbitrarily-renamed-webkit",
            "--browser-name=/tmp/build/arbitrarily-renamed-webkit",
