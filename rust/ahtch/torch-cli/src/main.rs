...
fn default_socket_path() -> PathBuf {
    match std::env::var_os("TMPDIR") {
        Some(tmp) => PathBuf::from(tmp).join("ahtchd.sock"),
        None => PathBuf::from("/tmp/ahtchd.sock"),
    }
}
...
/// (ahtchd.sock -> ahtchd.log) — must agree with the daemon's convention.
        ...
        match arg.as_str() {
            ...
            flag if flag.starts_with("--") => {
                ...
                match param {
                    ...
                    None => return Err(format!("unknown flag: --{name} (see ahtch {op} --help)")),
                }
            }
...
fn stdin_handles(n: usize, op: &str) -> Result<Vec<String>, String> {
    ...
    if std::io::stdin().is_terminal() {
        return Err(format!(
            "{op}: missing tensor operand(s) — pass handles as arguments or pipe them in (see ahtch {op} --help)"
        ));
    }
...
fn build_nn_request(args: &RawArgs) -> Result<serde_json::Value, String> {
    let kind = args
        .positionals
        .first()
        .ok_or("usage: ahtch nn <linear|relu|sigmoid|tanh|gelu|sequential|parameters|info> …")?;
    let mut nn_args = serde_json::Map::new();
    match kind.as_str() {
        "linear" => {
            let parse_feat = |index: usize, name: &str| -> Result<i64, String> {
                args.positionals
                    .get(index)
                    .ok_or(format!("nn linear: usage: ahtch nn linear <in> <out>"))?
                    .parse::<i64>()
        ...
        "layer_norm" => {
            let shape_text = args
                .positionals
                .get(1)
                .ok_or("usage: ahtch nn layer_norm '[shape]' [--eps E]")?;
        ...
        "save" | "load" => {
            let module = args
                ...
                .get(1)
                .cloned()
                .ok_or(format!("usage: ahtch nn {kind} <nn://module> <path>"))?;
            let raw_path = args
                .positionals
                .get(2)
                .cloned()
        ...
        "train" | "eval" => {
            let module = args
                ...
                .get(1)
                .cloned()
                .ok_or(format!("usage: ahtch nn {kind} <nn://module>"))?;
            Ok(serde_json::json!({
        ...
        "sgd" | "adam" | "adamw" | "rmsprop" => {
            let module = args
                ...
                .get(1)
                .cloned()
                .ok_or(format!("usage: ahtch nn {kind} <nn://module> [--lr …]"))?;
        ...
        "zero_grad" => {
            let handle = args
                ...
                .get(1)
                .cloned()
                .ok_or("usage: ahtch nn zero_grad <optim://…|nn://…>")?;
        ...
        "set_lr" => {
            let optimizer = args
                ...
                .get(1)
                .cloned()
                .ok_or("usage: ahtch nn set_lr <optim://…> <lr>")?;
            let lr: f64 = args
                .positionals
                .get(2)
                .ok_or("usage: ahtch nn set_lr <optim://…> <lr>")?
        ...
        "parameters" => {
            let module = args
                ...
                .get(1)
                .cloned()
                .ok_or("usage: ahtch nn parameters <nn://module>")?;
        ...
        "info" => {
            let module = args
                ...
                .get(1)
                .cloned()
                .ok_or("usage: ahtch nn info <nn://module>")?;
...
fn run_tensors(args: &RawArgs, socket: &PathBuf) -> Result<(), String> {
    ...
    if !args.positionals.is_empty() || args.flags.iter().any(|(n, _)| n != "json") {
        return Err("usage: ahtch tensors [--json]".to_string());
    }
    if !daemon_alive(socket) {
        if json_mode {
            println!("[]");
...
fn run_daemon_verb(args: &RawArgs, socket: &PathBuf) -> Result<(), String> {
    let verb = args
        .positionals
        .first()
        .ok_or("usage: ahtch daemon <status|ttl|stop|restart|start>")?;
    match verb.as_str() {
        "status" => {
            if !daemon_alive(socket) {
        ...
        "ttl" => {
            let duration = args
                .positionals
                .get(1)
                .ok_or("usage: ahtch daemon ttl <duration>  (e.g. 30m, 2h, none)")?;
            if !daemon_alive(socket) {
                return Err(format!("daemon not running (socket: {})", socket.display()));
            }
...
const PRODUCT_HELP_LEAD: &str = "Astrohacker Torch — GPU tensor operations from any shell";
const GENERAL_USAGE: &str = "usage: ahtch <op> [tensors...] [params...] [--flags]\n       ahtch ops              list available operations\n       ahtch <op> --help      usage for one operation\n       ahtch daemon <verb>    status | ttl | stop | restart | start";
...
fn print_op_help(op: &str) {
    match op {
        "tensor" => println!("usage: ahtch tensor <json-data> [--dtype <kind>]"),
        "value" => println!("usage: ahtch value [handle]   (or pipe the handle in)"),
        "shape" => println!("usage: ahtch shape [handle]   (or pipe the handle in)"),
        "daemon" => println!("usage: ahtch daemon <status|ttl|stop|restart|start>"),
        name => {
...
fn run() -> Result<(), String> {
    ...
    if op_name == "--help" || op_name == "help" || op_name == "-h" {
        println!("{PRODUCT_HELP_LEAD}");
        println!("{GENERAL_USAGE}");
        return Ok(());
    }
    if op_name == "--version" || op_name == "version" || op_name == "-V" {
...
const NU_PRELUDE: &str = r#"# ahtch.nu — GENERATED by `ahtch nu-module`; do not edit by hand.
...
/// Emit ahtch.nu: the prelude plus one wrapper per table op.
fn generate_nu_module() -> String {
    ...
    for spec in nutorch_ops::OPS {
        ...
        let invocation = format!("^ahtch {} {}", spec.name, body_args.join(" "));
...
mod nu_module_staleness {
    /// The committed ahtch.nu must equal the generator's output — the
    ...
    fn committed_module_is_current() {
        ...
        let committed = include_str!("../../ahtch.nu");
        assert_eq!(
            generated, committed,
            "ahtch.nu is stale: regenerate with `ahtch nu-module | save -f ahtch.nu`"
        );
    }
}
