    ...
    match kind {
        "linear" => {
            let in_features = int_arg("in_features").ok_or((
                "bad_argument",
                "nn linear: usage: ahtch nn linear <in> <out> [--no-bias] [--weight T] [--bias-tensor T]".to_string(),
            ))?;
        ...
        "conv1d" | "conv2d" | "conv_transpose2d" => {
            let req = |name: &str| -> Result<i64, (&'static str, String)> {
                int_arg(name).ok_or((
                    "bad_argument",
                    format!("nn {kind}: usage: ahtch nn {kind} <in_channels> <out_channels> <kernel_size> [flags]"),
                ))
            };
        ...
        "embedding" => {
            let num = int_arg("num_embeddings").ok_or((
                "bad_argument",
                "nn embedding: usage: ahtch nn embedding <num_embeddings> <embedding_dim>"
                    .to_string(),
            ))?;
        ...
        "layer_norm" => {
            let shape = match args.get("normalized_shape") {
                ...
                _ => {
                    return Err((
                        "bad_argument",
                        "nn layer_norm: usage: ahtch nn layer_norm '[shape]'".to_string(),
                    ))
                }
            };
        ...
        "batch_norm" => {
            let features = int_arg("num_features").ok_or((
                "bad_argument",
                "nn batch_norm: usage: ahtch nn batch_norm <num_features>".to_string(),
            ))?;
            let eps = float_module_arg(args, "eps", 1e-5);
        ...
        "group_norm" => {
            let groups = int_arg("num_groups").ok_or((
                "bad_argument",
                "nn group_norm: usage: ahtch nn group_norm <num_groups> <num_channels>"
                    .to_string(),
            ))?;
        ...
        "softmax" => {
            let dim = int_arg("dim").ok_or((
                "bad_argument",
                "nn softmax: usage: ahtch nn softmax <dim>".to_string(),
            ))?;
        ...
        "max_pool2d" | "avg_pool2d" => {
            let kernel = int_arg("kernel_size").ok_or((
                "bad_argument",
                format!("nn {kind}: usage: ahtch nn {kind} <kernel_size> [--stride S --padding P]"),
            ))?;
