...
static NSString *pdfCopyBridgeMode()
{
    if (!pdfCopyBridgeEnabled())
        return nil;
    NSString *mode = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_COPY_BRIDGE_MODE"];
    return mode.length ? mode : @"baseline";
}
...
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTION_SURFACE_TRACE"].length > 0;
}

static bool pdfSelectedTextRouteTraceEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTED_TEXT_ROUTE_TRACE"].length > 0;
}

static bool pdfSelectedTextCacheCopyEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTED_TEXT_CACHE_COPY"].length > 0;
}

static bool pdfSelectedTextCacheCopyTraceEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTED_TEXT_CACHE_COPY_TRACE"].length > 0;
}

static bool pdfSelectedTextStabilizedCaptureEnabled()
{
    return pdfSelectedTextCacheCopyEnabled() && NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTED_TEXT_STABILIZED_CAPTURE"].length > 0;
}

static bool pdfSelectionGeometryRemediationEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTION_GEOMETRY_REMEDIATION"].length > 0;
}
...
static NSString *pdfSelectionGeometryRemediationMode()
{
    if (!pdfSelectionGeometryRemediationEnabled())
        return nil;
    NSString *mode = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTION_GEOMETRY_REMEDIATION_MODE"];
...
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTION_STATE_TRANSITION"].length > 0;
}

static bool pdfPointerPrimeProductionDisabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_POINTER_PRIME_DISABLE"].length > 0;
}

static bool pdfHudSaveTraceEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_HUD_SAVE_TRACE"].length > 0;
}

static bool pdfActionProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_ACTION_PROBE"].length > 0;
}

static bool pdfStateOracleProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_STATE_ORACLE_PROBE"].length > 0;
}

static bool pdfVisualOracleProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_VISUAL_ORACLE_PROBE"].length > 0;
}

static bool pdfDirectCommandProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_DIRECT_COMMAND_PROBE"].length > 0;
}

static bool pdfPrintOperationProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_PRINT_OPERATION_PROBE"].length > 0;
}

static bool pdfPrintDialogProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_PRINT_DIALOG_PROBE"].length > 0;
}

static bool pdfPrintModalProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_PRINT_MODAL_PROBE"].length > 0;
}

static bool printPresentationProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PRINT_PRESENTATION_PROBE"].length > 0;
}

static bool nativeUiProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_NATIVE_UI_PROBE"].length > 0;
}

static bool syntheticPrintProbeEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_SYNTHETIC_PRINT_PROBE"].length > 0;
}

static bool pdfProductionZoomTraceEnabled()
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_PRODUCTION_ZOOM_TRACE_FILE"].length > 0;
}
...
static NSString *pdfSelectionStateTransitionMode()
{
    if (!pdfSelectionStateTransitionEnabled())
        return nil;
    NSString *mode = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTION_STATE_TRANSITION_MODE"];
...
{
    NSString *value = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTED_TEXT_CACHE_MAX_AGE"];
    if (!value.length)
        return 2.0;
...
static void appendPdfHudSaveTrace(NSString *line)
{
    if (!pdfHudSaveTraceEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_HUD_SAVE_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-hud-save-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfHudSave(WebContents *contents, NSString *phase, NSString *detail)
{
    if (!pdfHudSaveTraceEnabled())
        return;
    appendPdfHudSaveTrace([NSString stringWithFormat:@"surfari-pdf-hud-save tab=%d phase=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
...
{
    return NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_KEYBOARD_TRACE"].length > 0
        || NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_KEYBOARD_TRACE_FILE"].length > 0;
}
...
static void appendPdfKeyboardTrace(NSString *line)
{
    if (!pdfKeyboardTraceEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_KEYBOARD_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-keyboard-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfKeyboard(WebContents *contents, NSString *phase, NSString *detail)
{
    if (!pdfKeyboardTraceEnabled())
        return;
    appendPdfKeyboardTrace([NSString stringWithFormat:@"surfari-pdf-keyboard tab=%d phase=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
        detail ?: @""]);
...
static void appendPdfActionTrace(NSString *line)
{
    if (!pdfActionProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_ACTION_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-action-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfAction(WebContents *contents, NSString *phase, NSString *action, NSString *detail)
{
    if (!pdfActionProbeEnabled())
        return;
    appendPdfActionTrace([NSString stringWithFormat:@"surfari-pdf-action tab=%d phase=%@ action=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        action ?: @"none",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
...
static void appendPdfStateOracleTrace(NSString *line)
{
    if (!pdfStateOracleProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_STATE_ORACLE_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-state-oracle-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfStateOracle(WebContents *contents, NSString *phase, NSString *action, NSString *detail)
{
    if (!pdfStateOracleProbeEnabled())
        return;
    appendPdfStateOracleTrace([NSString stringWithFormat:@"surfari-pdf-state-oracle tab=%d phase=%@ action=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        action ?: @"none",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
...
static void tracePdfPasswordOracle(WebContents *contents, NSString *phase)
{
    ...
    [webView evaluateJavaScript:script completionHandler:^(id result, NSError *error) {
        ...
        appendPdfStateOracleTrace([NSString stringWithFormat:@"surfari-pdf-password-oracle tab=%d phase=%@ action=pdf-password url=%@ %@",
            tab,
            phaseCopy,
            url,
            detail]);
...
static void appendPdfFormOracleTrace(NSString *line)
{
    if (!pdfFormOracleTraceEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_FORM_ORACLE_TRACE_FILE"];
    if (!path.length)
        return;
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
...
static void tracePdfFormPdfKitOracle(WebContents *contents, NSString *phase)
{
    ...
    NSString *expectedValue = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_FORM_EXPECTED_VALUE"] ?: @"";
    ...
    if (!document) {
        appendPdfFormOracleTrace([NSString stringWithFormat:@"surfari-pdf-form-oracle tab=%d phase=%@ action=pdfkit url=%@ status=error reason=document-unavailable",
            contents->tab_id,
            phase ?: @"unknown",
            url]);
        return;
    ...
    appendPdfFormOracleTrace([NSString stringWithFormat:@"surfari-pdf-form-oracle tab=%d phase=%@ action=pdfkit url=%@ status=ok source=%@ page=%@ widget_count=%lu target_found=%d field=%@ type=%@ widget=%@ value_length=%lu expected_length=%lu value_equals_expected=%d read_only=%d",
        contents->tab_id,
        phase ?: @"unknown",
        url,
        source,
...
static void tracePdfFormOracle(WebContents *contents, NSString *phase)
{
    ...
    [webView evaluateJavaScript:script completionHandler:^(id result, NSError *error) {
        ...
        NSString *detail = error
            ? [NSString stringWithFormat:@"status=error error=%@", error.localizedDescription ?: @""]
            : [NSString stringWithFormat:@"status=ok state=%@", [result isKindOfClass:NSString.class] ? result : @""];
        appendPdfFormOracleTrace([NSString stringWithFormat:@"surfari-pdf-form-oracle tab=%d phase=%@ url=%@ %@",
            tab,
            phaseCopy,
            url,
            detail]);
...
static void dispatchPdfPasswordReturnKeyUp(WebContents *contents)
{
    ...
    [webView evaluateJavaScript:script completionHandler:^(id result, NSError *error) {
        ...
        appendPdfStateOracleTrace([NSString stringWithFormat:@"surfari-pdf-password-return-keyup tab=%d url=%@ %@",
            tab,
            url,
            detail]);
...
static void appendPdfVisualOracleTrace(NSString *line)
{
    if (!pdfVisualOracleProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_VISUAL_ORACLE_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-visual-oracle-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfVisualOracle(WebContents *contents, NSString *phase, NSString *action, NSString *detail)
{
    if (!pdfVisualOracleProbeEnabled())
        return;
    appendPdfVisualOracleTrace([NSString stringWithFormat:@"surfari-pdf-visual-oracle tab=%d phase=%@ action=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        action ?: @"none",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
...
static void appendPdfDirectCommandTrace(NSString *line)
{
    if (!pdfDirectCommandProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_DIRECT_COMMAND_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-direct-command-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfDirectCommand(WebContents *contents, NSString *phase, NSString *action, NSString *detail)
{
    if (!pdfDirectCommandProbeEnabled())
        return;
    appendPdfDirectCommandTrace([NSString stringWithFormat:@"surfari-pdf-direct-command tab=%d phase=%@ action=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        action ?: @"none",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
...
static void appendPdfPrintOperationTrace(NSString *line)
{
    if (!pdfPrintOperationProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_PRINT_OPERATION_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-print-operation-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfPrintOperation(WebContents *contents, NSString *phase, NSString *detail)
{
    if (!pdfPrintOperationProbeEnabled())
        return;
    appendPdfPrintOperationTrace([NSString stringWithFormat:@"surfari-pdf-print-operation tab=%d phase=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
        detail ?: @""]);
...
static void appendPdfPrintDialogTrace(NSString *line)
{
    if (!pdfPrintDialogProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_PRINT_DIALOG_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-print-dialog-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfPrintDialog(WebContents *contents, NSString *phase, NSString *detail)
{
    if (!pdfPrintDialogProbeEnabled())
        return;
    appendPdfPrintDialogTrace([NSString stringWithFormat:@"surfari-pdf-print-dialog tab=%d phase=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
        detail ?: @""]);
...
static void appendPdfPrintModalTrace(NSString *line)
{
    if (!pdfPrintModalProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_PRINT_MODAL_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-print-modal-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfPrintModal(WebContents *contents, NSString *phase, NSString *detail)
{
    if (!pdfPrintModalProbeEnabled())
        return;
    appendPdfPrintModalTrace([NSString stringWithFormat:@"surfari-pdf-print-modal tab=%d phase=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
        detail ?: @""]);
...
static void appendPrintPresentationTrace(NSString *line)
{
    if (!printPresentationProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PRINT_PRESENTATION_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-print-presentation-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePrintPresentation(WebContents *contents, NSString *phase, NSString *attempts)
{
    if (!printPresentationProbeEnabled())
        return;
    appendPrintPresentationTrace([NSString stringWithFormat:@"surfari-print-presentation tab=%d phase=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
...
static void appendNativeUiProbeTrace(NSString *line)
{
    if (!nativeUiProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_NATIVE_UI_TRACE_FILE"];
    if (!path.length)
        return;
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
...
static void traceNativeUiProbe(WebContents *contents, NSString *phase, NSString *attempts, NSString *detail)
{
    if (!nativeUiProbeEnabled())
        return;
    NSString *row = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_NATIVE_UI_ROW"];
    appendNativeUiProbeTrace([NSString stringWithFormat:@"surfari-native-ui-probe row=%@ tab=%d phase=%@ ppid=%d url=%@ %@ %@",
...
static bool publishNativeUiTrackedPid(NSString *trackedPath)
{
    if (!trackedPath.length)
        return false;
    NSString *line = [NSString stringWithFormat:@"surfari %d\n", getpid()];
...
static void performNativeUiProbe(WebContents *contents)
{
    ...
    NSString *tracePath = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_NATIVE_UI_TRACE_FILE"];
    NSString *trackedPath = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_NATIVE_UI_TRACKED_PIDS_FILE"];
    NSString *readyPath = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_NATIVE_UI_WATCHER_READY_FILE"];
    if (!tracePath.length)
        return;
    ...
    NSString *activationMode = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PRINT_PRESENTATION_ACTIVATION_MODE"];
    if ([activationMode isEqualToString:@"regular"]) {
        [NSApp setActivationPolicy:NSApplicationActivationPolicyRegular];
        [attempts addObject:@"setActivationPolicyRegular"];
    }
    NSString *windowMode = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PRINT_PRESENTATION_WINDOW_MODE"];
    if (contents->window) {
    ...
    alert.informativeText = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_NATIVE_UI_ROW"] ?: @"surfari-engine";
...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    [@"TermSurf Issue 834 Synthetic Print Probe" drawAtPoint:NSMakePoint(32, 48) withAttributes:attrs];
}
@end
...
static void appendSyntheticPrintTrace(NSString *line)
{
    if (!syntheticPrintProbeEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_SYNTHETIC_PRINT_TRACE_FILE"];
    if (!path.length)
        return;
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
...
static void traceSyntheticPrint(WebContents *contents, NSString *phase, NSString *attempts, NSString *detail)
{
    if (!syntheticPrintProbeEnabled())
        return;
    NSString *row = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_SYNTHETIC_PRINT_ROW"];
    NSString *mode = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_SYNTHETIC_PRINT_RUN_MODE"];
    appendSyntheticPrintTrace([NSString stringWithFormat:@"surfari-synthetic-print row=%@ mode=%@ tab=%d phase=%@ ppid=%d url=%@ %@ %@",
        row.length ? row : @"unknown",
...
static void performSyntheticPrintProbe(WebContents *contents)
{
    ...
    NSString *tracePath = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_SYNTHETIC_PRINT_TRACE_FILE"];
    NSString *trackedPath = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_SYNTHETIC_PRINT_TRACKED_PIDS_FILE"];
    NSString *readyPath = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_SYNTHETIC_PRINT_WATCHER_READY_FILE"];
    if (!tracePath.length)
        return;
    if (!trackedPath.length || !readyPath.length) {
...
static void appendPdfProductionZoomTrace(NSString *line)
{
    if (!pdfProductionZoomTraceEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_PRODUCTION_ZOOM_TRACE_FILE"];
    if (!path.length)
        return;
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
...
static void tracePdfProductionZoom(WebContents *contents, NSString *phase, NSString *action, NSString *detail)
{
    if (!pdfProductionZoomTraceEnabled())
        return;
    appendPdfProductionZoomTrace([NSString stringWithFormat:@"surfari-pdf-production-zoom tab=%d phase=%@ action=%@ url=%@ %@",
        contents ? contents->tab_id : 0,
        phase ?: @"unknown",
        action ?: @"none",
        contents && contents->web_view.URL.absoluteString ? contents->web_view.URL.absoluteString : @"",
...
{
    NSString *override = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_DOWNLOAD_DIR"];
    if (override.length)
        return override;
...
static NSString *uniquePdfSavePath(NSString *directory, NSString *filename, NSError **error)
{
    ...
    if (pathIsSymlink(directory)) {
        if (error)
            *error = [NSError errorWithDomain:@"TermSurfSurfariPDFSave" code:6 userInfo:@{ NSLocalizedDescriptionKey: @"refusing symlink download directory" }];
        return nil;
    }
    ...
    for (NSUInteger index = 0; index < 1000; index++) {
        ...
        if (!pathIsInsideDirectory(candidate, directory)) {
            if (error)
                *error = [NSError errorWithDomain:@"TermSurfSurfariPDFSave" code:1 userInfo:@{ NSLocalizedDescriptionKey: @"candidate outside download directory" }];
            return nil;
        }
        if (pathIsSymlink(candidate)) {
            if (error)
                *error = [NSError errorWithDomain:@"TermSurfSurfariPDFSave" code:2 userInfo:@{ NSLocalizedDescriptionKey: @"refusing symlink destination" }];
            return nil;
        }
    ...
    if (error)
        *error = [NSError errorWithDomain:@"TermSurfSurfariPDFSave" code:3 userInfo:@{ NSLocalizedDescriptionKey: @"no available unique filename" }];
    return nil;
}
...
static NSString *savePdfDataToDownloads(NSData *data, NSString *suggestedFilename, NSError **error)
{
    if (!data.length) {
        if (error)
            *error = [NSError errorWithDomain:@"TermSurfSurfariPDFSave" code:4 userInfo:@{ NSLocalizedDescriptionKey: @"empty PDF data" }];
        return nil;
    }
    ...
    if (!pathIsInsideDirectory(standardDestination, directory)) {
        ...
        if (error)
            *error = [NSError errorWithDomain:@"TermSurfSurfariPDFSave" code:5 userInfo:@{ NSLocalizedDescriptionKey: @"written path escaped download directory" }];
        return nil;
    }
...
static void tracePdfFind(WebContents *contents, NSString *phase, NSString *detail)
{
    if (!contents)
        return;
    NSString *line = [NSString stringWithFormat:@"surfari-pdf-find tab=%d phase=%@ url=%@ query={%@} %@",
        contents->tab_id,
        phase ?: @"unknown",
...
static void appendPdfSelectionSurfaceTrace(NSString *line)
{
    if (!pdfSelectionSurfaceTraceEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTION_SURFACE_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-selection-surface-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void appendPdfSelectedTextRouteTrace(NSString *line)
{
    if (!pdfSelectedTextRouteTraceEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTED_TEXT_ROUTE_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-selected-text-route-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void appendPdfSelectedTextCacheCopyTrace(NSString *line)
{
    if (!pdfSelectedTextCacheCopyTraceEnabled())
        return;
    NSString *path = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_SELECTED_TEXT_CACHE_COPY_TRACE_FILE"];
    if (!path.length)
        path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"termsurf-surfari-pdf-selected-text-cache-copy-trace.log"];
    NSString *entry = [line stringByAppendingString:@"\n"];
    NSData *data = [entry dataUsingEncoding:NSUTF8StringEncoding];
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *parent = path.stringByDeletingLastPathComponent;
...
static void tracePdfSelectionSurface(WebContents *contents, NSString *phase)
{
    ...
    appendPdfSelectionSurfaceTrace([NSString stringWithFormat:
        @"surfari-pdf-selection-surface-summary tab=%d phase=%@ url=%@ focused=%d gui_active=%d object_count=%lu window=%@ key_window=%d main_window=%d app_key_window=%@ app_main_window=%@ web_view=%@ hit_target=%@ first_responder=%@ responder_chain=%@ clipboard={%@}",
        contents->tab_id,
        phase ?: @"unknown",
        contents->web_view.URL.absoluteString ?: @"",
    ...
    for (id object in objects) {
        appendPdfSelectionSurfaceTrace([NSString stringWithFormat:
            @"surfari-pdf-selection-surface-object tab=%d phase=%@ index=%lu object=%@ superclass_chain=%@ %@ selectors={%@} safe_values={%@} accessibility={%@} methods={%@} properties={%@} ivars={%@}",
            contents->tab_id,
            phase ?: @"unknown",
            (unsigned long)index++,
...
{
    NSString *mode = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_VIEWPORT_FIX_PROBE"];
...
static void tracePdfViewportFix(WebContents *contents, NSString *phase, NSString *detail)
{
    ...
    appendPdfViewGeometryTrace([NSString stringWithFormat:
        @"surfari-pdf-viewport-fix tab=%d phase=%@ url=%@ %@",
...
static void clearPdfSelectedTextCache(WebContents *contents, NSString *reason)
{
    ...
    if (pdfSelectedTextCacheCopyTraceEnabled()) {
        appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
            @"surfari-pdf-selected-text-cache tab=%d action=clear reason=%@ had_cache=%d generation=%llu cache_generation=%llu epoch=%llu consumed=%d text={%@}",
            contents->tab_id,
            reason ?: @"unknown",
...
static void requestPdfSelectedTextCacheCaptureSample(WebContents *contents, NSString *phase, uint64_t generation, uint64_t epoch, NSString *url, NSTimeInterval delaySeconds, bool stabilized)
{
    ...
    appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
        @"surfari-pdf-selected-text-cache tab=%d action=%@ phase=%@ delay=%.3f url=%@ generation=%llu epoch=%llu pdf_url=%d",
        contents->tab_id,
    ...
    if (!currentPdfAllowsCopying(contents, &copyPermissionReason)) {
        appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
            @"surfari-pdf-selected-text-cache tab=%d action=capture-skip reason=copy-restricted detail=%@",
            contents->tab_id,
    ...
    @try {
        ...
        fn(contents->web_view, getSelectedText, ^(NSString *text) {
            ...
            if (reason) {
                appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
                    @"surfari-pdf-selected-text-cache tab=%d action=%@ reason=%@ phase=%@ delay=%.3f generation=%llu epoch=%llu text={%@}",
            ...
            appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
                @"surfari-pdf-selected-text-cache tab=%d action=%@ phase=%@ delay=%.3f generation=%llu epoch=%llu replace=%d previous={%@} text={%@}",
                capturedContents->tab_id,
            ...
            if (stabilized && replace) {
                appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
                    @"surfari-pdf-selected-text-cache tab=%d action=capture-accept phase=%@ generation=%llu text={%@}",
                    capturedContents->tab_id,
                    phaseCopy,
                    (unsigned long long)generation,
                    routeTraceStringSample(value)]);
        ...
        appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-cache tab=%d action=capture-exception phase=%@ exception=%@", contents->tab_id, phase ?: @"unknown", exception.name]);
...
static void requestPdfSelectedTextCacheCapture(WebContents *contents, NSString *phase)
{
    ...
    appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
        @"surfari-pdf-selected-text-cache tab=%d action=stabilized-capture-start phase=%@ generation=%llu epoch=%llu url=%@ deadline=%.3f",
        contents->tab_id,
        phase ?: @"unknown",
        (unsigned long long)generation,
        (unsigned long long)epoch,
    ...
    for (NSNumber *delayNumber in delays) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (delay >= 0.70 && capturedContents && capturedContents->pdf_selected_text_cache_epoch == epoch) {
                ...
                appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
                    @"surfari-pdf-selected-text-cache tab=%d action=stabilized-capture-complete phase=%@ generation=%llu epoch=%llu cached={%@}",
                    capturedContents->tab_id,
                    phaseCopy,
                    (unsigned long long)generation,
                    (unsigned long long)epoch,
...
static void applyPdfSelectedTextCacheCopyIfNeeded(WebContents *contents, NSString *phase)
{
    ...
    appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
        @"surfari-pdf-selected-text-cache tab=%d action=copy-decision phase=%@ replace=%d reason=%@ normal={%@} cached={%@} generation=%llu cache_generation=%llu",
        contents->tab_id,
        phase ?: @"unknown",
        replace ? 1 : 0,
        reason ?: @"unknown",
    ...
    if (replace) {
        ...
        appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
            @"surfari-pdf-selected-text-cache tab=%d action=copy-replace phase=%@ final={%@}",
            contents->tab_id,
            phase ?: @"unknown",
            clipboardSample()]);
    }
...
static void schedulePdfSelectedTextCacheCopyDecision(WebContents *contents, NSString *phase)
{
    ...
    appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:
        @"surfari-pdf-selected-text-cache tab=%d action=copy-schedule phase=%@ delay=%.3f wait_reason=%@ stabilization_active=%d deadline=%.3f epoch=%llu",
        contents->tab_id,
        phaseCopy,
        delay,
        waitReason,
...
static void tracePdfSelectedTextRoutes(WebContents *contents, NSString *phase)
{
    ...
    appendPdfSelectedTextRouteTrace([NSString stringWithFormat:
        @"surfari-pdf-selected-text-route-summary tab=%d phase=%@ url=%@ web_view=%@ clipboard={%@}",
        contents->tab_id,
        phase ?: @"unknown",
    ...
    if ([webView respondsToSelector:getSelectedText]) {
        appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-start tab=%d phase=%@ route=getSelectedText responds=1", contents->tab_id, phase ?: @"unknown"]);
        @try {
            typedef void (*GetSelectedTextFn)(id, SEL, void (^)(NSString *));
            ...
            fn(webView, getSelectedText, ^(NSString *text) {
                appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-result tab=%d phase=%@ route=getSelectedText text={%@}", tab, phaseCopy, routeTraceStringSample(text)]);
            });
        } @catch (NSException *exception) {
            appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-result tab=%d phase=%@ route=getSelectedText exception=%@", contents->tab_id, phase ?: @"unknown", exception.name]);
        }
    } else {
        appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-start tab=%d phase=%@ route=getSelectedText responds=0", contents->tab_id, phase ?: @"unknown"]);
    }
    ...
    if ([webView respondsToSelector:selectedRangeWithCompletionHandler]) {
        appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-start tab=%d phase=%@ route=selectedRangeWithCompletionHandler responds=1", contents->tab_id, phase ?: @"unknown"]);
        @try {
            ...
            fn(webView, selectedRangeWithCompletionHandler, ^(NSRange range) {
                appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-result tab=%d phase=%@ route=selectedRangeWithCompletionHandler location=%lu length=%lu", tab, phaseCopy, (unsigned long)range.location, (unsigned long)range.length]);
            });
        } @catch (NSException *exception) {
            appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-result tab=%d phase=%@ route=selectedRangeWithCompletionHandler exception=%@", contents->tab_id, phase ?: @"unknown", exception.name]);
        }
    } else {
        appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-start tab=%d phase=%@ route=selectedRangeWithCompletionHandler responds=0", contents->tab_id, phase ?: @"unknown"]);
    }
    ...
    if ([webView respondsToSelector:unionRectInVisibleSelectedRange]) {
        @try {
            ...
            appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-result tab=%d phase=%@ route=unionRectInVisibleSelectedRange rect={%@}", contents->tab_id, phase ?: @"unknown", routeTraceRectSummary(rect)]);
        } @catch (NSException *exception) {
            appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-result tab=%d phase=%@ route=unionRectInVisibleSelectedRange exception=%@", contents->tab_id, phase ?: @"unknown", exception.name]);
        }
    } else {
        appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-start tab=%d phase=%@ route=unionRectInVisibleSelectedRange responds=0", contents->tab_id, phase ?: @"unknown"]);
    }
    ...
    if ([webView respondsToSelector:writeSelectionToPasteboard]) {
        @try {
            ...
            appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-result tab=%d phase=%@ route=writeSelectionToPasteboard ok=%d pasteboard=%@ text={%@}", contents->tab_id, phase ?: @"unknown", ok ? 1 : 0, pasteboardName, routeTraceStringSample(text)]);
            [pasteboard clearContents];
        } @catch (NSException *exception) {
            appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-result tab=%d phase=%@ route=writeSelectionToPasteboard exception=%@", contents->tab_id, phase ?: @"unknown", exception.name]);
        }
    } else {
        appendPdfSelectedTextRouteTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-route-start tab=%d phase=%@ route=writeSelectionToPasteboard responds=0", contents->tab_id, phase ?: @"unknown"]);
    }
}
...
static void tracePdfNavigationDiagnostics(WebContents *contents, NSString *phase, NSString *url)
{
    ...
    if (pdfSelectedTextRouteTraceEnabled()) {
        appendPdfSelectedTextRouteTrace([NSString stringWithFormat:
            @"surfari-pdf-selection-navigation tab=%d phase=%@ pid=%d url=%@ web_view=%@ window=%@ first_responder=%@ responder_chain=%@ ca_context=%u pdf_url=%d",
            contents->tab_id,
            phase ?: @"unknown",
            getpid(),
...
static bool performPdfPrintModalDiagnostic(WebContents *contents, int keycode, int modifiers)
{
    ...
    NSString *readyPath = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_PDF_PRINT_MODAL_WATCHER_READY_FILE"];
...
static void traceCopyState(WebContents *contents, NSString *label)
{
    ...
    appendPdfCopyTrace([NSString stringWithFormat:
        @"surfari-pdf-copy-state tab=%d label=%@ url=%@ focused=%d gui_active=%d window=%@ key_window=%d main_window=%d app_key_window=%@ web_view=%@ web_frame=%@ first_responder=%@ responder_chain=%@ hit_target=%@ target_nil=%@ target_webview=%@ target_hit=%@ validate_nil={%@} validate_webview={%@} validate_hit={%@} clipboard={%@}",
        contents->tab_id,
        label,
        contents->web_view.URL.absoluteString ?: @"",
        contents->focused ? 1 : 0,
...
static void tracePdfCopyBridgeState(WebContents *contents, NSString *phase, NSString *mode, PdfCopyBridgeState *state)
{
    if (!pdfCopyTraceEnabled() || !contents)
        return;
    appendPdfCopyTrace([NSString stringWithFormat:
        @"surfari-pdf-copy-bridge-state tab=%d phase=%@ mode=%@ active=%d original_allow_key_main=%d current_allow_key_main=%d original_key_window=%@ original_main_window=%@ original_first_responder=%@ current_key_window=%@ current_main_window=%@ host_key=%d host_main=%d first_responder=%@ responder_chain=%@ clipboard={%@}",
        contents->tab_id,
        phase ?: @"unknown",
        mode ?: @"none",
...
static void runPdfCopyBridgePreKeyRoutes(WebContents *contents, NSString *mode, NSEvent *event)
{
    ...
    if ([mode isEqualToString:@"send-action-first"]) {
        ...
        appendPdfCopyTrace([NSString stringWithFormat:@"surfari-pdf-copy-bridge-route tab=%d mode=%@ route=sendActionNil ok=%d clipboard={%@}", contents->tab_id, mode, ok_nil ? 1 : 0, clipboardSample()]);
        BOOL ok_webview = [NSApp sendAction:copySelector to:contents->web_view from:nil];
        appendPdfCopyTrace([NSString stringWithFormat:@"surfari-pdf-copy-bridge-route tab=%d mode=%@ route=sendActionWebView ok=%d clipboard={%@}", contents->tab_id, mode, ok_webview ? 1 : 0, clipboardSample()]);
        ...
        appendPdfCopyTrace([NSString stringWithFormat:@"surfari-pdf-copy-bridge-route tab=%d mode=%@ route=windowPerformKeyEquivalent ok=%d clipboard={%@}", contents->tab_id, mode, ok_window ? 1 : 0, clipboardSample()]);
        BOOL ok_webview = [contents->web_view performKeyEquivalent:event];
        appendPdfCopyTrace([NSString stringWithFormat:@"surfari-pdf-copy-bridge-route tab=%d mode=%@ route=webViewPerformKeyEquivalent ok=%d clipboard={%@}", contents->tab_id, mode, ok_webview ? 1 : 0, clipboardSample()]);
...
static void tracePdfSelectionGeometryRemediation(WebContents *contents, NSString *phase, NSString *mode, NSString *reason, NSPoint originalLocal, NSPoint adjustedLocal, NSPoint originalWindow, NSPoint adjustedWindow, bool appliedGeometry, bool appliedFocus)
{
    ...
    appendPdfViewGeometryTrace([NSString stringWithFormat:
        @"surfari-pdf-selection-remediation tab=%d phase=%@ gate=1 mode=%@ reason=%@ url=%@ pdf_url=%d original_local=%@ adjusted_local=%@ original_window=%@ adjusted_window=%@ target_y=%.2f edge_avoidance=%d focus_action=%d geometry_action=%d first_responder=%@ responder_chain=%@",
        contents->tab_id,
        phase ?: @"unknown",
        mode ?: @"none",
        reason ?: @"unknown",
...
static void tracePdfSelectionStateTransition(WebContents *contents, NSString *phase, NSString *mode, NSString *reason, NSPoint userLocal, NSPoint primeLocal, bool focusAction, bool pointerAction, bool clampAction)
{
    ...
    appendPdfViewGeometryTrace([NSString stringWithFormat:
        @"surfari-pdf-selection-state-transition tab=%d phase=%@ source=%@ gate=1 mode=%@ reason=%@ url=%@ generation=%llu consumed=%d pending=%d pdf_url=%d user_local=%@ prime_local=%@ focus_action=%d pointer_action=%d clamp_action=%d coordinate_changed=0 first_responder=%@ responder_chain=%@",
        contents->tab_id,
        phase ?: @"unknown",
        source,
        mode ?: @"none",
...
static bool hasSurfariRenderProofColor(int magenta, int cyan, int yellow, int webkit_green)
{
...
static void classifySnapshotImage(WebContents *contents, NSString *method, NSImage *image, NSError *error)
{
    ...
    NSString *status = hasSurfariRenderProofColor(magenta, cyan, yellow, webkit_green) ? @"pass" : @"blank";
...
static bool snapshotHasRenderProof(NSImage *image, NSError *error)
{
    ...
    return hasSurfariRenderProofColor(magenta, cyan, yellow, webkit_green);
}
...
static bool isAllowedLocalHttpsServerTrustChallenge(NSURLAuthenticationChallenge *challenge)
{
    ...
    NSString *allowedHost = environment[@"TERMSURF_SURFARI_TEST_ALLOW_LOCAL_HTTPS_CERT_HOST"];
    if (![allowedHost isEqualToString:space.host])
        return false;

    NSString *allowedPort = environment[@"TERMSURF_SURFARI_TEST_ALLOW_LOCAL_HTTPS_CERT_PORT"];
...
void ts_forward_mouse_event(ts_web_contents_t wc, int type, int button, int x, int y, int click_count, int modifiers)
{
    ...
    if (pdfCopyTraceEnabled()) {
        appendPdfCopyTrace([NSString stringWithFormat:@"surfari-pdf-copy-mouse tab=%d type=%d button=%d x=%d y=%d click_count=%d modifiers=%d location=%@ original_location=%@ edge_mode=%@ edge_delta=%.2f remediation_geometry=%d", contents->tab_id, type, button, x, y, click_count, modifiers, NSStringFromPoint(location), NSStringFromPoint(original_location), pdfSelectionEdgeProbeMode() ?: @"none", pdfSelectionEdgeDeltaX(), geometryRemediationApplied ? 1 : 0]);
    }
    ...
    if (type == 1) {
        ...
        if (was_dragging && contents->pdf_selected_text_drag_exceeded_threshold)
            ...
            appendPdfSelectedTextCacheCopyTrace([NSString stringWithFormat:@"surfari-pdf-selected-text-cache tab=%d action=capture-skip reason=drag-threshold generation=%llu", contents->tab_id, (unsigned long long)contents->pdf_selected_text_generation]);
    }
    ...
    size_t out_path_len)
{
    ...
    NSString *previousDownloadDir = NSProcessInfo.processInfo.environment[@"TERMSURF_SURFARI_DOWNLOAD_DIR"];
    setenv("TERMSURF_SURFARI_DOWNLOAD_DIR", download_dir, 1);
    ...
    if (previousDownloadDir.length)
        setenv("TERMSURF_SURFARI_DOWNLOAD_DIR", previousDownloadDir.UTF8String, 1);
    else
        unsetenv("TERMSURF_SURFARI_DOWNLOAD_DIR");
